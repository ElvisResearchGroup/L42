...
public class CircularityIssue {
  ...
  private void checkNotIllTyped(P.NCs pi,HashSet<P.NCs>visited){
    ...
    if(!novel){return;}
    CoreL l=(CoreL)p.of(pi,e.poss());//propagate errors for path not existent
    if(!l.info().isTyped()){reportError(pi,l);}
    for(var pj:l.info().typeDep()){checkNotIllTyped(p.from(pj,pi),visited);}
    }
  public void reportError(P.NCs path,CoreL l0) {
  ...
  public void reportError(HashSet<P> untypedDeps,P.NCs path,CoreL l){
    assert !l.info().isTyped();
    for(var pi:l.info().typeDep()){
      pi=p.from(pi,path);
      var li=(CoreL)p.of(pi,l.poss());
