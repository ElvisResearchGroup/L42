...
ContinueFloat
<ol>
WMenuItem(`#HttpRequest',`The Http request library')

</ol>
CFloat

</p><p id="HttpRequest">
WBigTitle(The Http request library)
WTitle(Importing process and example usage)

OBCode
HttpR = Load:{reuse[L42.is/HttpRequest]}
..
  (
  )
CCode
...
WComm can be seen at https://raw.githack.com/Language42/is/main/testing/HttpRequest.xhtml
