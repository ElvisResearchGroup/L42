...
extends AtomicTest.Tester{public static Stream<AtomicTest>test(){return Stream.of(new AtomicTest(()->
   ...
   fail("Void.This3",ErrMsg.notValidC("Void"))
   ),new AtomicTest(()->
   pass("A(B)","A(B)")   
   ),new AtomicTest(()->
   pass("!A(B)","!A(B)")   
   ...
   fail("({method Any m()={} #norm{}})",ErrMsg.malformedCoreFullL()+Err.hole+ErrMsg.malformedCoreMWT("#norm{}","[m()]"))
   ),new AtomicTest(()->
   ...
   fail("{method A fwd imm(B x)=x.imm(x)}",ErrMsg.invalidMethodName("fwd imm"))
