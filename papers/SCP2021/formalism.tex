...
\begin{figure}
    ...
    \mid \M{l}{e_1}{e_2}\mid\Kw{try}^{\s}\oC e_1\cC\ \Kw{catch}\ \oC e_2\cC
		...
		\seguitoProduzione{
			...
			\text{\IOComm{Did I split this up into two lines correctly?}}
	}{}\\
		\produzione{\error}{
			\EV[\M{l}{v}{\Kw{false}}]
			,\quad\!\!\text{where }
...
\item $\ER=\EV[\M{l}{v}{\EV'[\hole\singleDot f]}]$, and this $hole\singleDot f$ expression came from the body of the $\invariant$ method itself.
\end{iitemize}
\IOComm{bassically this has $\EV, r_L$ before, I've simplified it greatly now}
%\loseSpace
Note that this only holds when the very next reduction we are about to perform is $l.\invariant$ or $l\singleDot f$.
...
In particular, no method (other than the \emph{injected} invariant check itself) can see an object who is currently being checked for validity.
