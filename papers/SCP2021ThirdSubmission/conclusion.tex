...
\section{Conclusion}
\label{s:conclusion}
In this paper we (1) \IO[7]{identified language features that soundly} support representation invariants in object-oriented verification; (2) presented a full formalism for our approach with capabilities that is proved to soundly guarantee that all objects involved in execution are valid; (3) conducted extensive case studies
%, of which the GUI one is highlighted in the paper,
 showing that we require orders of magnitude fewer runtime checks than \textit{visible state semantics} and  \IO[29.3]{approximately $31\%$ fewer annotations (with $3\,\nicefrac{1}{2}$ times fewer tokens)\footnote{\IO[29.4]{Calculated by combining the counts from our GUI, Hamster, and Family case studies.}} than equivalent versions in Spec\#.} We hope that as a result of this work, the software verification community will make more use of the advanced general purpose language features, such as capabilities, appearing in modern languages to achieve its goals.
...
Our work builds on a specific form of reference and object capabilities, whose
popularity is growing, and we expect future languages to support some variations of these.
Crucially, any language already designed with such a support
can also support our invariant protocol with minimal added complexity.



% Mention flexible ownership types as a potential solution? (Assuming it is)
...
% \LINE
% The language we presented here restricts the form of \Q@invariant()@ and rep mutator methods. 
...
%Soundly catching exceptions like stack overflows or out of memory cannot be allowed in \Q@invariant()@ methods, since they are not deterministically thrown.
...
%aims to be efficient by limiting the number of validation calls, however we have \REVComm{no empirical evaluation of our approach's performance}{3}{\label{CONTRA1}contradictory to [see footnote \autoref{CONTRA2}]}.
...
Their struggle is proof of how hard it is to deal with the expressive power of unrestricted imperative object-oriented programming.
 Here instead we build on languages using reference and object capabilities to tame the use of imperative features. In this way
