...
\section{Conclusion}
\label{s:conclusion}
In this paper we (1) identified language features that soundly support representation invariants in object-oriented verification; (2) presented a full formalism for our approach with capabilities that is proved to soundly guarantee that all objects involved in execution are valid; (3) conducted extensive case studies
%, of which the GUI one is highlighted in the paper,
 showing that we require orders of magnitude fewer runtime checks than \textit{visible state semantics} and approximately $31\%$ fewer annotations (with $3\,\nicefrac{1}{2}$ times fewer tokens)\footnote{Calculated by combining the counts from our GUI, Hamster, and Family case studies.} than equivalent versions in Spec\#. We hope that as a result of this work, the software verification community will make more use of the advanced general purpose language features, such as capabilities, appearing in modern languages to achieve its goals.
