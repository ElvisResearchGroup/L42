...
%By looking to a survey by Voigt \etal~\cite{Voigt2013} and
By looking to surveys ~\cite{Voigt2013,falcone:hal-01882410}
and
 the extensive MOP project~\cite{meredith2012overview},
it seems that most runtime verification tools empower users
...
Many works attempt to move out of the `runtime verification tool' philosophy to ensure runtime verification monitors work as expected, as for example
%\sepItems
...
relying on powerful aspect-oriented support, they detect any field update in the whole reachable object graph of any object, and check all the invariants that such update may have violated.
...
However, their approach addresses neither exceptions nor non-determinism caused by I/O, so their soundess guarantee does not scale to programs using such features.
...
Using reference capabilities to enforce strong exception safety is a much simpler alternative, providing the same level of safety, albeit being more restrictive.
...
%one time at the end of each setter, rep mutator method and constructor.
