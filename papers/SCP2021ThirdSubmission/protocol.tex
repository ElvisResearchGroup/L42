...
we can not do the same for a doubly linked lists of \IO[4.1]{unenscapsulated} mutable elements.
\IO[4.2]{In order for the nodes to be referenced in the \Q!invariant! method, since they contain mutable data, they must be reacheable from a \Q!rep! field, but then the elements of the list cannot be made accesible as \Q!mut! from outside the list. However, we could use the transform pattern presented in \autoref{s:patterns} to mutate the elements of such a list, in a way that does not allow aliases to be saved outside.\footnote{\IO[4.3]{If our protocol was extended to support polymorphic reference capabilities (as in Gordon \etal's work~\cite{GordonEtAl12}), we could allow a reference with a polymorphic reference capability to be reachable from a \Q!rep! field, provided that the state reachable from such a reference cannot be read from the \Q!invariant! method. This could be done by supporting a reference capability that prevents reading state (such as the \Q!tag! capability of Pony~\cite{clebsch2015deny}), and requiring that such a polymorphic reference capability be type checked as if it were \Q!tag!.
Outside the list however, if the polymorphic reference capability is known to be \Q!mut!, the elements could then be freely accessed and aliased, as the \Q!invariant! would be guaranteed not to depend on them.}}
We do not however} make it harder to correctly implement such data structures.
