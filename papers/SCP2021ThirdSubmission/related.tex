...
\item \IO[31.1]{The invariant of each object passed to a method\footnote{\IO[31.2]{or function/subprogram}} call, and every parameter and result upon a method return, must be ensured:
either for every method ~\cite{ACSL,WhyML}; only for methods declared within the same scope as the invariant's declaration, but visible outside~\cite{Ada}; or for such methods as well as all calls that are within the scope of the invariant's declaration, but are to methods outside~\cite{Spark}.}
\item \IO{The same as above, but an invariant may be declared as ``strong'', requiring that it must hold for every variable/paramater at every step of execution (a ``sequence point")~\cite{ACSL}.}
