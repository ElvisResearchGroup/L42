...
In particular, the current concrete L42 syntax uses the \Q@capsule@ keyword to ensure various properties of a field.
The language relies on the presence of annotations or other specific methods
to decide what restrictions to apply and properties to ensure. In this case, the presence of the \Q!@Cache.Now! annotation clarifies that the field \Q@capsule Items items@
...
\item Upon calling a method (a.k.a a function/subprogram), the invariant of each parameter (and part/field of a parameter) must be shown to hold;
...
\item The same as above, but an invariant may optionally be declared `strong', requiring that it must hold for every variable/parameter (and their parts) at every well-defined step of execution (a `sequence point')~\cite{ACSL}.
...
Note that all, except the last, of those approaches fail our strict requirements and allow for broken objects to be observed.
...
However, a study~\cite{chalin2007logical} discovered that developers expect short-circuit semantics and arithmetic exceptions in specification languages to follow the semantics of the underlying language; thus for example \Q@1/0@\,\Q@||@\,\Q@2>1@ should not hold, while \Q@2>1@\,\Q@||@\,\Q@1/0@ should, thanks to short circuiting.
