...
\IO[11]{To ensure that $t$ and $f$ themselves are evaluated if and only if $x$ is ``true'', the \Q!Bool.i${}f! method could instead require an object with an \Q!apply! method, whose body will be $t$ or $f$, respectively. If we added syntax sugar for lambdas, as in Java 8, we could then call it with
\Q@$x$.i${}$f(() -> $t$, () -> $f$)@ %TODO: FIX!!
}}
...
\begin{itemize}
	...
	\IO[21]{For example, we could have $\s|\try{\e}{e'} \rightarrow \s|\trys{\s}{e}{e'} \rightarrow^{*} \s,\s' |\trys{\s}{\error}{e'} \rightarrow \s,\s' | e' \rightarrow^{*} \s'',\s' | e"$. Thus the body of the \Q!try! ($e$) has not modified $\s$, but it may have created new objects, which will be in $\s'$; the \Q!catch! block on the other hand ($e'$) can freely mutate $\s$ into $\s''$. 
	Note that it will not be able to read or mutate any of the objects in $\s'$, however this property is not neccessary, and need not hold if user-defined objects could be thrown.}
