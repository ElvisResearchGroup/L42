...
\IO[11]{To ensure that $t$ and $f$ themselves are evaluated if and only if $x$ is ``true'', the \Q!Bool.i${}$f! method could instead require an object with an \Q!apply! method, whose body will be $t$ or $f$, respectively. If we added syntax sugar for lambdas, as in Java 8, we could then do \Q@$x$.i${}$f(() -> $\ t$, () -> $\ f$).apply()@}% Marco: check this
}
