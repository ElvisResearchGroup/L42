...
In a purely functional setting, \IO[12]{once the programmer has written the invariant code itself,} sound runtime checking is trivial: the runtime simply needs to check the invariant each time a value/object is created (or in the case of refinement types, converted to such a type).
...
With appropriate type annotations (\IO[13]{namely, reference capabilities on fields, method receivers, paramaters, and return types}), the code of \Q@Range@ and \Q@BoxRange@ is accepted as correct by our system: no matter how \Q@Range@ objects are used, a broken \Q@Range@ object will never be involved in execution. \IO[1]{In particular, our system ensures that the \Q!Range.set! method cannot pass \Q!this!, or an alias to \Q!this!, to the \Q!box.set! method.} \IO[24]{Note that our system does not require that the \Q!min! and \Q!max! fields use primitive immutable number types, in fact, they could store complex (and possibly cyclic) mutable data; our system will ensure that this data cannot be mutated, except within the \Q!Range.set! method (or other similar methods within the enclosing \Q!Range!). If we were to not require the box pattern, we would likely make the language much more complicated, we instead choose to add a few simple to describe restrictions that can be added on top of pre-existing type systems.}
