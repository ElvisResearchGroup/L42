...
In a purely functional setting, \IO[12]{once the programmer has written the invariant code itself,} sound runtime checking is \IO[21]{easy}: the runtime simply needs to check the invariant each time a value/object is created (or in the case of refinement types, converted to such a type).
...
However, if \Q@min@ is greater than or equal to \Q@this.max@, \Q@set@ would violate our stricter approach. The execution of
...
With appropriate type annotations (\IO[13]{namely, reference capabilities on fields, method receivers, paramaters, and return types}), the code of \Q@Range@ and \Q@BoxRange@ is accepted as correct by our system: no matter how \Q@Range@ objects are used, a broken \Q@Range@ object will never be involved in execution. \IO[1]{In particular, our system ensures that the \Q!Range.set! method cannot pass \Q!this!, or an alias to \Q!this!, to the \Q!box.set! method.}\footnote{\IO[24]{Note that our system does not require that the \Q!min! and \Q!max! fields use primitive immutable number types, in fact, they could store complex (and possibly cyclic) mutable data; our system will ensure that this data can only be mutated within the \Q!Range.set! method (or other similar methods within the enclosing \Q!Range!). We would like to keep the language as simple as possible, and so we have opted to add simple restrictions over pre-existing type systems, instead of trying to find an alternative design that doesn't require such a box pattern.}}s
...
\IO[10]{which used runtime instrumentation to determine if a memory update has violated the invariant of any live object, even if it is not reachable from the current stack frame}.
...
\begin{itemize}
	...
	\item \autoref{s:protocol} fully explains our novel invariant protocol, and our novel field kind for mutable data.
	...
	\item \autoref{s:case-studyAll} contains many case studies, showing that our protocol is \IO[28.2]{}more succinct than the pack/unpack approach and much more efficient then the visible state semantic.
