...
In a purely functional setting, the programmer only needs to write the code for the invariant check itself, then the runtime needs to call this code each time a value/object is created (or in the case of refinement types, converted to such a type).
...
Also note that many VMs and compilers can optimise away wrapper objects in many circumstances.~\cite{Bolz:2011:ARP:1929501.1929508}
This is even more applicable in languages with inlined structs, like C++ or C\#.
}

With appropriate type annotations (simple keywords attached to fields, method receivers, paramaters, and return types), the code of \Q@Range@ and \Q@BoxRange@ is accepted as correct by our system: no matter how \Q@Range@ objects are used, a broken \Q@Range@ object will never be involved in execution. In particular, our system ensures that the \Q!Range.set! method cannot pass \Q!this!, or an alias to \Q!this!, to the \Q!box.set! method.\footnote{Note that our system does not require that the \Q!min! and \Q!max! fields use primitive immutable number types, in fact, they could store complex (and possibly cyclic) mutable data; our system will ensure that this data can only be mutated within the \Q!Range.set! method (or other similar methods within the enclosing \Q!Range!). We would like to keep the language as minimal as possible, and so we have opted to not add a separate concept of immutable \emph{classes}.}
...
which used runtime instrumentation to determine if a memory update has violated the invariant of any live object, even if it is not reachable from the current stack frame.
...
Our implementation works by checking that a given class conforms to our protocol, and injecting invariant checks in the appropriate places.
