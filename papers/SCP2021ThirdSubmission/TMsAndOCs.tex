...
\IO[14]{For example, following L42's approach:}
\begin{lstlisting}
...
Those \emph{encapsulated} fields \IO[25]{(which do not yet support destructive reads)} \IO[3]{facilitate parallelism and can model various forms of ownership.}%
...
\IO[26.2]{This works by checking method signatures and bodies against specific syntactic patterns. For example,
with the appropriate restrictions, a method body consisting of a sequence of calls to \Q!mut! methods on \Q!capsule! fields can have these calls parallelised.}
...
these reference capabilities are opt-in: \IO[15]{a programmer can choose to use them only when desired,} for example they can write many classes
...
\IO[16]{Consider the following example:}
\begin{lstlisting}
