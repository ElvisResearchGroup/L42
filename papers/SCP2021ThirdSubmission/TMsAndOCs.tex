...
%\end{itemize}
\noindent There are only two kinds of objects: mutable and immutable, but there are more kinds of reference capabilities.
In L42 only \Q@mut@ and \Q@imm@ references can be saved on the heap: \Q@capsule@ and \Q@read@ references only exists on the stack.
...
%\noindent
In L42, a \Q!capsule! local variable always holds a \Q!capsule! reference: this is ensured by allowing them to be read only once (similar to linear and affine types~\cite{boyland2001alias}). For example:\SS
\begin{lstlisting}
...
Those \emph{encapsulated} fields (which do not support destructive reads) facilitate parallelism and can model various forms of ownership.%
\footnote{It may seem surprising that 
those weaker forms of encapsulation are still sufficient to ensure safe unobservable parallelism.
...
programmers can write many classes
...
For example, imagine a program where most objects belong to classes designed without worrying about ownership, aliasing and encapsulation and with most methods requiring mutation.
...
class Point {
	...
	mut method Void add(imm Int d) throws MyChecked {
		this.x += d; this.y += d;
		if (...) { throw new MyUnchecked(); }//Always ok throwing unchecked exceptions
		if (...) { throw new MyChecked(); }//Ok: `MyChecked' is in the `throws'
	}}
...
try {
	mut Point p = new Point(1, 2)
	try {
		p.add(someNumber); // could throw a MyChecked, or any unchecked exception
		if (...) { p = new Point(...); } // update a local variable
	} 
	catch (MyChecked e) { ... }// Ok
	// Adding the following `catch' would be a type error,
	// as the body of the try needs `p' to be non-final and mut:
	// catch (MyUnchecked e) { ... }
	...
	try { if (p.x != p.y) { throw new MyUnchecked(); } }
	catch (MyUnchecked e) { // Ok:
		// the try part can be typed where `p' is seen as final and read
		// we can be sure the value of `p' has not changed
		// but we can change it here
		p = new Point(...) // or p.x+1;
	}
}
catch (MyUnchecked e) { ... } // ok, `p' is guaranteed to be unreachable
\end{lstlisting}

Similarly to Java, L42 distinguishes between checked and unchecked exceptions, strong exception safety is only enforced for unchecked exceptions, and so
