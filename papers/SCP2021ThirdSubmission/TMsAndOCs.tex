...
%\end{itemize}
\noindent \IO[2]{Thus}, there are only two kinds of objects: mutable and immutable, but there are more kinds of reference capabilities.
In L42 only \Q@mut@ and \Q@imm@ references can be saved on the heap\IO[2]{:} \Q@capsule@ and \Q@read@ references only exists on the stack.
...
Those \emph{encapsulated} fields \IO[3]{facilitate parallelism and can model various forms of ownership.}
...
Similarly to Java, L42 distinguishes between checked and unchecked exceptions, \IO[2]{strong exception safety is only enforced for unchecked exceptions, and so}
