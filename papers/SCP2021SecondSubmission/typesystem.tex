...
\noindent Note that we always have $\demut{\emptyset} = \emptyset$, $\demut{\demut{\G}} = \demut{\G}$, and \IO{$\G(x) \leq \demut{\G}(x)$.}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\IO{The full L42 language supports more promotions, such as \Q!read! to \Q!imm!, these could be added to our type system, but would greatly complicate our proofs. The \textsc{TAsCapsule} rule should be sufficient to demonstrate that our invariant protocol can be supported in a system with promotions.}
...
\SS\begin{proof}
	...
	\IO{f $\ty[\s][\demut{\G}]e{T'''}$, we can turn such a typing derivation into one for $\ty e{T'''}$,\\
...
\noindent \IO{Now an auxiliary lemmas that is needed to reason over the types of runtime introduced monitor expressions, they are well typed provided the body is well typed.}
...
\SS\begin{theorem}[\IO{Type Preservation}]\ \\
	\indent If $\vdash\s$, $\tyr eT$ and $\s|e\rightarrow^n\s'|e'$, then $\vdash\s'$
	and $\tyr[\s']{e'}T$.
\end{theorem}
\SS\begin{proof}
	\IO{The proof is by induction on $n$. In the first base case, we assume $n = 0$ and the conclusion trivially holds since $\s' = \s$ and $e' = e$.}

	\IO{In the second base case, we assume $n = 1$,} i.e. $\s|e\rightarrow\s'|e'$. We note by
	$\thm{Type Rule}$ that we have $\tyer{e}{T'}$, for some $T' \leq T$.
	We will then show that $\tyr[\s']{e'}{T'}$ holds by induction
	on the size of $e$.

	In the base case \IO{for our inner induction}, we assume that there is no $\EV$ and $e_0$ where
	...
	In the inductive case for \IO{our inner induction}, we have some $e_0$ and minimal $\EV \neq \hole$, where $e = \EV[e_0]$; thus $e_0$ is a direct sub-expression of $e$.
	...
	\IO{Now we can use the same typing rule that gave us $\tyr{\EV[e_0]}{T'}$ to obtain $\tyr[\s']{\EV[e'_0]}{T'}$, as required; this step is valid since:	\SS
	\begin{itemize}
	...
	\item The typing rule may require other premises, each of form $\tyr[\s']{e_1}{T_1}$, where $e_1$ is a direct subexpressions of $\EV$. Since $\tyr{\EV[e_0]}{T'}$, we must also have $\tyr{e_1}{T_0}$. Regardless of the reduction rule applied to get $\s|e_0\rightarrow\s'|e'_0$, we have $\forall l\in\dom(\s)$, $\C{l} = \C[\s']{l}$, and so we also have $\tyr[\s']{e_1}{T_1}$ (since the only typing rule that depends on the $\s$ is the $\textsc{TRef}$ rule, but since we have not altered the value of any $\C{l}$, such a rule is still valid under $\s'$).
...
%		\item Change the premise for the sub-expression in the hole of $\EV$, which will be of form $\tyr{e_0}{T_0}$, to be $\tyr[\s']{e'_0}{T''}$.\SS
...
%		\item Each side-condition is still valid, as they do not depend on the value of $\s$ nor the values of any sub-expressions (this holds as we have preserved the \emph{types} of these sub-expressions).
%	\end{iitemize}
%	Thus we have $\tyr{\EV[e'_0]}{T'}$ as required.

	\noindent Finally, in the inductive case \IO{of our outer induction}, we have $n = k+1$ and $\s|e\rightarrow^{k}\s_k|e_k\rightarrow\s'|e'$.
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			where $\s = \s'[l'.f = l'']$ and $e = \M{l'}{\Kw{mut}\,l'}{\invariant{l'}}$
				...
				\TODO{FUCK we have $l\in\rog(\s,l'')$, HOW DO I PROVE that $l\in\rog(\s',l'')$?
					Does that even hold}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Suppose $\E = \EV[\E'']$ for some $\E''$, thus $\E''[\Kw{capsule}\,l] = e'$:
					...
					Suppose $i = 0$.
						By the inductive hypothesis we thus have $\encap(\s,\EV[\call{\hole}m{\drange{\mdf}l}],l)$.
						...
						Since $\mdf'_i = \Kw{capsule}$ and $i = 0$, the method was not a rep mutator,
						...
						for $\mdf'_j\,l_j$)
						As our type system requires that each method bodies mentions $\Kw{capsule}$
						receivers at most once, it follows that $\Kw{this}\notin\E'''$.
						Since $\E' = \as{\E'''[\Kw{this}\coloneqq\mdf'_0\,l_0,\trange x{\!\coloneqq\mdf'}l]}{\mdf''}$,
						it follows that $l'$ is not $\reach$ through $\E'$.

					Now suppose $i\geq1$.
						By the inductive hypothesis, we have $\encap(\s,\EV[\call{\mdf_0\,l_0}m{\drange[][1][i-1]{\mdf}l,\hole,\drange[][i+1][n]{\mdf}l}],l)$.
						As in the $i = 0$ case, it follows from $\thm{Mut Consistency}$
						...
						Since our type system requires that each method bodies mentions each
						$\Kw{capsule}$ parameter at most once, it follows that $x_i\notin\E'''$.
						Since $l'$ is not $\reach$ through $l_0$, and $\E'\in\{\as{\E''''}{\mdf''},\,\M l{\as{\E''''}{\mdf''}}{\invariant{l_0}}\}$,
						where $\E'''' = \E''''[\Kw{this}\coloneqq\mdf'_0\,l_0,\trange x{\!\coloneqq\mdf'}l]$,
						it follows that $l'$ is not $\reach$ through $\E'$.

					Either way, as $l'$ is not $\reach$ through $\E'$, and it was
