...
\IOComm{I made this lemma take an empty $\s$, to make the proofs simpler.}%
...
\SS\begin{proof}
	...
	\begin{itemize}
		...
		\item \IO{Suppose the $\textsc{TRef}$ or $\textsc{TMonitor}$ rules applied, then we would have an $l \in \dom(\emptyset)$, a contradiction.}
...
\end{theorem}
\SS\begin{proof}
	\IO{We prove this by induction on the number of reductions since
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			where $\s = \s'[l'.f = l'']$ and\\$e = \EV[\M{l'}{\Kw{mut}\,l'}{\invariant{l'}}]$.
				...
				\IO{Suppose $l \notin \rog(\s', l'')$.
				But the only difference between $\s$ and $\s'$ is at $l'.f$.
	...
	\item Now we show that \thm{Capsule Consistency} holds.
		\IO{Suppose $e = \E[\Kw{capsule}\,l]$, for some $\E$,} and $\encap(\s,\E,l)$
		doesn't hold.
		\LSenum
		...
		\begin{itemize}
			\item \IO{$\crule{new/new true}{\s'}{\EV[e'']}{\s}{\E[\Kw{capsule}\,l]}$, where
			$\s = \s',l_0\mapsto C\{\ls\}$, $\E[\Kw{capsule}\,l] = \EV[e']$, $e'\in\{\M{l_0}{\Kw{mut}\,l_0}{\invariant{l_0}},\,\Kw{mut}\,l_0\}$,
			and $e'' = \new C{\vs}$.}
			\begin{itemize}
				\item \IO{Suppose $\E$ is of form $\EV[\E']$, i.e. the hole in $\E$ is within
				$e'$.
					But there are no $\Kw{capsule}$s in $e'$, a contradiction.}

				\item \IO{Otherwise, $\E$ is not of form $\EV[\E']$, i.e. the hole in $\E$
				is within $\EV$, and so $\Kw{capsule}\,l\in\EV$ and $e'\in\E$.
					...
					\IO{Suppose $l'$ is $\reach$ through $\EV$, that overlaps with \E}, then there is some $l''\in\EV$
					with $l'\in\rog(\s,l'')$. By $\thm{No Dangling}$, $l''\in\dom(\s')$,
					and since preexisting memory wasn't modified, it follows that $l'\in\rog(\s',l'')$;
					\IO{since $l''$ is in the part of $\EV$ that overlaps with $E$, which is identical to the part of $\EV$ that overlaps with $\E'$,
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item \IO{Now consider any $l_4$ where $\reach(\s_0,\EV,l_4)$.}
						\LSiiitem
						...
						\IO{Thus we must have $\s_0(l_4) = \s(l_4)$.}
						\LSiiitem
						
					\item \IO{By the above, reduction cannot have modified memory in such a way as to make something \reach in $\s|\EV$ that was not previously \reach in $\s_0|\EV$.
					...
					Suppose $\mdf'''' = \Kw{capsule}$.
							...
							and hence \IO{not $\reach(\s_0,\EV,l')$.
					...
					Otherwise, $\mdf'''' = \Kw{read}$.
						    ...
						    %\IO{Consider our reduction sequence }
						    
							\IO{By $\thm{Type Consistency}$ and our typing rules, it follows that during reduction,
