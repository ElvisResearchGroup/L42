...
is seen as a \Q@Void@ pre-cached value that is always kept up to date whenever the object is visible.
That is, invariants are supported with a mechanism similar to the one required to compute and keep caches up to date.
 L42 also supports other kinds of cached methods, which get computed in parallel when an instance of the corresponding class is created.
