...
\begin{itemize}
    ...
    %'one of kind' Marco: you wrote 'kind' but now kind is also a concept in our language
...
\begin{itemize}
	...
	\item Rep mutators must also follow the requirements in \ref{s:protocol}, except that a \Q!mut! method that reads a \Q!rep! field is \emph{always} considered a rep mutator, even if it only needs to use the field value as \Q!read!\footnote{
		...
		$\method{\Kw{read}\,}{\,\Kw{read}\,C}{m}{}{\Kw{this}\D f}$, where $C.f = \field{\Kw{rep}}{C}{f}$, and then call \Q@this.m()@ on a \Q!mut! \Q!this!.}
...
\end{figure}%
Our reduction rules are defined in Figure \ref{f:reductions}. We use the function $\fr(\s)$ to return an arbitrary $l$ such that $l \notin \dom(\s)$.
