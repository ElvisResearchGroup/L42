...
\begin{figure}
	...
	\begin{grammatica}
		...
		\produzione{A}{
			\amethod{\mdf}{T}{m}{\drange{T}{x}}
		}{abstract method}\\

		\produzione{M}{
			\method{\mdf}{T}{m}{\drange{T}{x}}{e}
		}{method}\\
...
We use $\mdf$ for our reference capabilities, and $\fmdf$ for field kinds. Recall that we don't model traditional \Q!capsule! fields, but instead model our novel \Q!rep! fields, which can only be initialised/updated with \Q!capsule! values.
...
\begin{itemize}
	...
	%This is similar to Pony's promote expressions. It is up to the typesystem what promotions it accepts as valid, provided they cannot be used to violate our requirements in \ref{s:proof}. 
...
\begin{itemize}
	...
	\SS[1]\begin{iitemize}
	\item $\rmdf{\Kw{imm}}{\fmdf} = \rmdf{\mdf}{\Kw{imm}} = \Kw{imm}$\SS
	\item $\rmdf{\Kw{read}}{\fmdf} = \Kw{read}$, if $\fmdf \neq \Kw{imm}$\SS
	\item $\rmdf{\Kw{mut}}{\Kw{rep}} = \rmdf{\Kw{mut}}{\Kw{mut}} = \Kw{mut}$\SS
	\item $\rmdf{\Kw{capsule}}{\Kw{rep}} = \rmdf{\Kw{capsule}}{\Kw{mut}} = \Kw{capsule}$
	\end{iitemize}
\end{itemize}
...
\begin{figure}
	...
	\\[0ex]\smash{
		...
		{\s}{\as{e[\Kw{this}\coloneqq\mdf_0\,l_0,\trange{x}{\!\coloneqq\mdf}{l}]}{\mdf'}}
		\text{, where:}}
	\\\qquad\begin{array}{l}
		\C{l_0}.m = \method{\mdf_0}{\mdf'\,\_}{m}{\drange{\mdf}{\_\,x}}{e}\\*
		\text{if }\mdf_0 = \Kw{mut}\text{, }\IO{\nexists f,\E\text{ with }\C{l_0}.f = \field{\Kw{rep}}{\_}{f}\text{ and }e = \E[\Kw{this}\D f]}
	\end{array} % because otherwise there seems to be too much space between this and as
	\\\rowSpace\smash{
		\rrule{call mutator}
		{\s}{\call{\_\,l_0}{m}{\range{\_\,l}}}
		{\s}{\M{l_0}{e}{\invariant{l_0}}}
		\text{, where:}}
	\\[\baselineskip]\qquad\begin{array}{l}
		\C{l_0}.m = \method{\Kw{mut}}{\mdf'\,\_}{m}{\drange{\mdf}{\_\,x}}{\E[\Kw{this}\D f]}\\*
		\C{l_0}.f = \field{\Kw{rep}}{\_}{f}\\*
		e = \as{\E[\Kw{this}\D f][\Kw{this}\coloneqq\Kw{mut}\,l_0,\trange{x}{\!\coloneqq\mdf}{l}]}{\mdf'}
...
\end{figure}%
Our reduction rules are defined in Figure \ref{f:reductions}. \IO{We use the function $\fr(\s)$ to return an arbitrary $l$ such that $l \notin \dom(\s)$}.
...
\IO{Note that although we do not define what $\fr$ actually returns, since it is a \emph{function} these reduction rules are deterministic: $l_0$ is uniquely defined for any given $\s$. }
...
\IO{Note that as \Q!Cap! has no \Q!rep! fields and can have multiple definitions of the same method, the \textsc{call} rule allows for non-determinism, but only if the receiver is of class \Q!Cap! and the method is a \Q!mut! method.}
...
%%\IO{Blahblah blah, why we need this?
