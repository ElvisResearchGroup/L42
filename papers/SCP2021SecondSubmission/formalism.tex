...
A typesystem, such as our example one in \autoref{s:typesystem}, may impose additional restrictions on method bodies, for example that they are well typed.
Our typesystem requirements in \autoref{s:proof} however only refer to the main expression, and hence only the methods that could actually be called need to be restricted.
