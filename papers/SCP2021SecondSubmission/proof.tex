...
\item $\reach(\s, e, l')$, $\exists f$ such that $\Cs{l'}.f = \field{\Kw{imm}}{\_}{f}$, and $l \in \rog(\s, \s[l'.f])$.
\end{iitemize}
...
\begin{iitemize}
	\item $l' = l$; or
	\item $\exists f$ such that $\Cs{l}.f \in \{\field{\Kw{rep}}{\_}{f},\field{\Kw{mut}}{\_}{f}\}$, and $l' \in \mrog(\s, \s[l.f])$
\end{iitemize}
...
\indent $\encap(\s, \E, l)$ iff $\forall l' \in \rog(\s, l)$, if $\muty(\s, \E[l], l')$, then 
not $\reach(\s, \E, l')$.
...
\SS\begin{Requirement}[Subject Reduction]\rm
	If $\VS(\s, \E[e])$, then $\tyr{e}{T}$.
\end{Requirement}
...
\SS\begin{Requirement}[Type Consistency]\rm\ 
\begin{ienumerate}
%if S; G; E |- e.f = e' : _ C, and C.f = T', then S; G; E[e.f = []] |- e' : T'
\item If \ty{\new{C}{C}{e_1,\ldots,e_n}}{T} then \ty{e_i}{T_i} where $T_i = C.i$.
\item If \ty{e}{\_\,C} and $C.f = \field{?}{T'}{f}$, then \ty{e'}{T'}.
\item If \ty{e}{\_\,C} and
$C.m = \method{\mdf}{\T}{m}{\T_1\,\x_1,\ldots,\T_n\x_n}{\_}$, then:
\begin{enumerate}
\item \ty{e}{\mdf\,C},
\item \ty{e_i}{T_i}, and
\item \ty{\call{e}{m}{e_1,\ldots,e_n}}{T}.
%\item hello
\end{enumerate}
\end{ienumerate}
\end{Requirement}%

\noindent We also assume that any expression inside a method body can be typed with the same reference capabilities as when it is expanded by our \textsc{call} rule (in particular any $x$ that is now an $l$ will have the same $\mdf$):%
\SS\begin{Requirement}[Method Consistency]\rm\
If $\VS(\s, \EV[\call{l}{m}{v_1,\ldots,v_n}])$ %and $\s|\EV[\call{l}{m}{v_1,\ldots,v_n}] \rightarrow \s|$,
where:
\begin{iitemize}
\item $\tyr{l}{\_\,C}$, $C.m = \method{\mdf}{\_}{m}{T_1\,x_1,\ldots T_n\,x_n}{\E[e]}$, 
\item $\E'$ = $\M{l}{\E}{l.\invariant}$ if $C.m$ is a rep mutator, otherwise $\E' = \E$,
\item $\G = \Kw{this} : \mdf\,C,x_1:T_1, \ldots, x_n:T_n$, and $e' = e[\Kw{this}\coloneqq l,x_1\coloneqq v_1,\ldots,x_n\coloneqq v_n]$,
\end{iitemize}

\indent then $\tyr{e'}{\mdf\,\_}$ implies $\ty[\emptyset]{e}{\mdf\,\_}$.
\end{Requirement}
...
\SS\begin{Requirement}[Imm Consistency]\rm\ \\
...
\SS\begin{Requirement}[Mut Consistency]\rm If $\VS(\s, \EV[e])$, \\
...
\SS\begin{Requirement}[Capsule Consistency]\rm\ 
\begin{ienumerate}
...
\SS\begin{Requirement}[Mut Update]\rm If \ty{e.f \equals e'}{T}, then \ty{e}{\Kw{mut}\,\_}.
\end{Requirement}

\noindent We additionally require that field accesses only be typed as \Q!mut!, if their receiver is also \Q!mut!:%
\SS\begin{Requirement}[Mut Access]\rm If \ty{e.f}{\Kw{mut}\,\_}, then \ty{e}{\Kw{mut}\,\_}.
\end{Requirement}
...
\SS\begin{Requirement}[Read Consistency]\rm\ 
\begin{ienumerate}
\item If $\G(x) = \Kw{read}\,\_$, then $\nty{x}{\Kw{mut}\,\_}$.
\item If \ty{e}{\_\,C} and $C.m = \method{\mdf}{\Kw{read}\,C'}{\_}{\_}$, then $\nty{\call{e}{m}{\es}}{\Kw{mut}\,\_}$.
\end{ienumerate}
...
\SS\begin{Requirement}[Strong Exception Safety]\rm
If $\VS(\s', \E[\trys{\s_0}{e_0}{e_1}])$, then\\
\indent $\forall l \in \dom(\s_0)$, not $\muty(\s, e_0, l)$.
\end{Requirement}
...
\end{Lemma}\SS
\begin{proof}
By $\VS$ we have $c\mapsto\Kw{Cap}\{\}|e_0\rightarrow^+ \s|e$, so we proceed by induction on the number of ``$\rightarrow$''s: in the base case, $e = e_0$ and so it cannot contain a monitor expression by the definition of \VS. If this property holds for $\VS(\s, e)$ but not for $\s'|e'$ with $\s|e\rightarrow \s'|e'$, we must have applied the \textsc{update}, \textsc{call}, or \textsc{new} rules; since no other reduction steps introduce a monitor expression. If the reduction was a \textsc{new}, $l$ will be fresh, so it could not have been in $\s_0$. If the reduction was an \textsc{update}, by \thm{Mut Update}, $l$ must have been \Q!mut!, similarly (by our well-formedness rules on method bodies) \textsc{call} will only introduce a monitor over a call to a \Q!mut! method, so by \thm{Type Consistency}, $l$ was \Q!mut!; either way we have that $l$ was \muty, since our reductions never change the $\s_0$ annotation, by \thm{Strong Exception Safety}, we have that $l \notin \s_0$.
\end{proof}
...
\end{Lemma}\SS
\begin{proof}
To prove this, we will use induction on the number of ``$\rightarrow$''s, and additionally prove that $\forall l' \in \dom(\s)$, not $\muty(\s', e', l')$, i.e. no part of the original $\s$ will become $\muty$ from the body of the call $l.\invariant$.

\emph{Base case}: If $\s|\EV[l.\invariant] \rightarrow \s'|\EV[e']$, then the reduction was performed by \textsc{call}.
By our well-formedness rules, the \Q!invariant()! method takes a \Q!read! \Q!this!, so by \thm{Method Consistency} and \thm{Read Consistency}, we have that $l$ is not \muty in $e'$ (since $l$ is typeable as \Q!read!). By our well-formedness rules on method bodies and \textsc{call}, we have that no other $l'$ was introduced in $e'$, thus nothing is $\muty$ in $e'$.

The only non-deterministic single reduction steps are for calls to \Q!mut! methods on a \Q!Cap!; however \Q!invariant()! is a \Q!read! method, so even if $l$ = $c$, we have $\s|\EV[l.\invariant] \Rightarrow \s'|\EV[e']$. Since \textsc{call} does not mutate $\s'$, we also have $\s' = \s$.
Additionally, since $l'$ is the only location reference in the expression $l.\invariant$ and it is not $\muty$, we additionally have $\forall l' \in \dom(\s)$, not $\muty(\s', e', l')$.
...
\indent $f \in \rf(\s, l)$ iff $\Cs{l}.f = \field{\Kw{rep}}{\_}{f}$.

\noindent An $l$ is \RNC if it is not reachable from its \Q!rep! fields:\\
\indent $\RNC(\s, l)$ iff $\forall f \in \rf(\s, l)$, $l \notin \rog(\s, \s[l.f])$.

\noindent We say that an $l$ is \WE if none of its \Q!rep! fields is \muty without passing through $l$:\\
\indent $\WE(\s, e, l)$ iff $\forall f \in \rf(\s, l)$, not $\muty(\s\setminus l, e, \s[l.f])$.

\noindent We say that an $l$ is \NCM if we aren't in a monitor for $l$ which must have been introduced by \textsc{call}, and we don't access any of it's \Q!rep! fields as \Q!mut!:\\
\indent $\NCM(\s, e, l)$ iff $\forall \E$:
\begin{iitemize}
\item if $e = \E[\M{l}{e'}{\_}]$, then $e' = l$, and
\item if $e = \E[l.f]$, $f \in \rf(\s, l)$, and \ntyr[\h.f]{l}{\Kw{capsule}\,\_}, then \ntyr{l.f}{\Kw{mut}\,\_}.
\end{iitemize}
...
\begin{iitemize}
\item $e' = \E[l.f]$, $f \in \rf(\s, l)$, and not $\reach(\s, \E, l)$ or
...
\SS\begin{theorem}[Rep Field Soundness]\rm
...
\item (\textsc{new}) $\s|\EV[\new{C}{v_1,\ldots,v_n}]\rightarrow \s'| \EV[\M{l}{l}{l\singleDot\invariant}]$, where $\s' = \s,l\mapsto C\{v_1,\ldots,v_n\}$:
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Now suppose $l' = l$ and consider each $i$ with $C.i = \field{\Kw{rep}}{\_}{f}$. By \thm{Type Consistency} and \thm{Capsule Consistency}, $v_i$ was \encap and $\rog(\s, v_i)$ is not \muty from \EV, and so $v_i$ is not $\muty(\s'\setminus l, \EV[\M{l}{l}{l.\invariant}], v_i)$; thus \WE holds for $l$ and each of its \Q!rep! fields.
...
\begin{enumerate}
	...
	\begin{itemize}
		\item Suppose $l' = l$ and $f \in \rf(\s, l)$, by \thm{Mut Access}, either $\ntyr{\s[l.f]}{\Kw{mut}\,\_}$ or $\tyr{l}{\Kw{capsule}\,\_}$. If $l$ was \Q!capsule!, then by \thm{Capsule Consistency} and \RNC, $l$ is not \reach from $\EV[\s[l.f]]$, so it is irrelevant if $l$ is no longer \WE. Otherwise, if $l$ was not \Q!capsule!, $\s[l.f]$ will not be \Q!mut!, so \WE is preserved for $l$.
		\item Now consider any $l'$ that was not \HNO, then by the inductive hypothesis, it was \WE and \NCM. Since this reduction doesn't modify memory, by \thm{Mut Consistency}, there is no other way to make the \rog of a \Q!rep! field $f'$ of $l'$ \muty without going through $l'$, so \WE is preserved for $l'$.
		\item Since this reduction doesn't modify memory, and by \thm{Mut Consistency}, we can't have made the \rog of any other \Q!rep! field $f'$ of any $l'$ \muty without going through $l'$, so \WE is preserved.
	...
	\begin{itemize}
		\item If $l' = l$, then $\EV = \EV'[\M{l}{\E[l.f]}{\_}]$, with $l$ not \reach from $\E$, and $l.f$ is an access to a \Q!rep! field. By \RNC, $l$ is not in the \rog of $\s[l.f]$, and so $l$ is not \reach from $\E[\s[l.f]]$, and so it is still \HNO.
...
\begin{enumerate}
	\item By the inductive hypothesis we have that \RNC holds for $l$. If $f \in \rf(\s, l)$, by \thm{Mut Update}, we have that $l$ is \muty, so by \thm{Type Consistency} and \thm{Capsule Consistency}, $\encap(\s, \EV[l.f = \square], v)$, hence $l$ is not \reach from $v$, and so after the update, \RNC still holds for $l$. In addition, if $f \notin \rf(\s, l)$ we obviously must still have \RNC. 	\item Otherwise, by the inductive hypothesis, $l'$ was \HNO, and so $l' \notin \rog(\s, v)$, so we can't have added $l'$ to the \rog of anything, thus \RNC still holds.
	\item By the inductive hypothesis we have that \RNC holds for each $l'$:
	\begin{itemize}
		\item If $f \in \rf(\s, l)$: by \thm{Mut Update}, we have that $l$ is \muty, so by \thm{Type Consistency} and \thm{Capsule Consistency}, $\encap(\s, \EV[l.f = \square], v)$, hence $l$ is not \reach from $v$, and so after the update, \RNC still holds for $l$.
	...
	\item Consider any $l'$ that was \WE and \NCM:
	\begin{itemize}
		\item If $l' = l$ and $f \in \rf(\s, l)$, by \thm{Type Consistency} and \thm{Capsule Consistency}, $v$ is \encap, thus $v$ is not \muty from \EV, and $l$ is not \reach from $v$, thus $v$ is still \encap and \WE still holds for $l$ and $f$.
		\item Now consider any $f' \in \rf(\s, l')$, with $l'.f' \neq l.f$; by the above, $l$ is \RNC and so $l \notin \rog(\s, \s[l'.f'])$. If $f$ was a \Q!mut! or \Q!rep! field, by \thm{Type Consistency} and \thm{Capsule Consistency}, $v$ was \Q!mut!, so by \WE, $v \notin \rog(\s, \s[l'.f'])$; thus we can't have made $\rog(\s, \s[l'.f'])$ \muty through $l.f$; so $l'.f'$ can't now be \muty through $l$. By \thm{Mut Consitency}, we couldn't have have made $l'.f'$ \muty some other way, so $l'$ is still \WE.
...
\item (\textsc{call}) $\s|\EV[\call{l}{m}{v_1,\ldots,v_n}]\rightarrow \s|\EV[e]$:
\begin{enumerate}
	\item As above, since this rule doesn't mutate memory, by the inductive hypothesis, every $l'$ must still be \RNC.
	\item Consider any $l'$ that was \WE and \NCM:
	\begin{itemize}		
		...
		\begin{itemize}
			\item Suppose $l' = l$. Since $m$ is not a rep mutator, if $e = \E[l.f]$, for some $f \in \rf(\s, l)$, we must have that $m$ was not a \Q!mut! method. So by \thm{Mut Access} and \thm{Method Consistency}, we have that $\ntyr{l.f}{\Kw{mut}\,\_}$ only if $m$ was a \Q!capsule! method, which by \thm{Method Consistency}, would mean that $\tyr{l}{\Kw{capsule}\,\_}$. So regardless of what fields $e$ accesses on $l$, we can't have broken \NCM for $l$.
...
\begin{ienumerate}
	...
	\begin{itemize}
	\item[] By \VS and our well-formedness requirements on method bodies, the monitor expression must have been introduced by \textsc{update}, \textsc{call}, or \textsc{new}. In each case the third expression started off as $l.\invariant$, and it has now (eventually) been reduced to $\Kw{true}$, thus by \thm{Determinism} $l$ is \valid. This rule does not modify pre-existing memory, introduce pre-existing $l$s into the main expression, nor remove monitors on other $l$s, thus every other pre-existing $l'$ is still \valid (due to \thm{Determinism}), or \mony.
	\end{itemize}

		\item (\textsc{new}) $\s|\EV[\new{C}{\vs}]\rightarrow \s,l\mapsto C\{\vs\}|\EV[ \M{l}{l}{l.\invariant}]$:
		\begin{itemize}
...
\indent If $\VS(\s, e)$, $\forall f,l$, if $\reach(\s, e, l)$, $\Cs{l}.f = \field{\Kw{imm}}{\_}{f}$, then $l \notin \rog(\s, \s[l.f])$.
\end{Lemma}\SS
\begin{proof}
...
\begin{iitemize}
	...
	\item $\ER = \E[\M{l}{e_1}{\E'}]$, and thus $l$ was found inside $e_2$. By our reduction rules, all monitor expressions start with $e_2=l.\invariant$; if this has yet to be reduced, then $\E' = \E''[\hole.\invariant]$, thus $\ER[l]$ is \trusted. The next execution step will be an \textsc{call}, so by our well-formedness rules for \Q!invariant()!, $e_2$ will only contain $l$ as the receiver of a field access; so if we just performed said \textsc{call}, $\E' = \E''[\hole.f]$: hence $\ER[l]$ is trusted. Otherwise, by \thm{Imm Not Circular}, \thm{Rep Field Soundness}, and \RNC, no further reductions of $e_2$ could have introduced an occurrence of $l$, so we must have that $l$ was introduced by the \textsc{call} to \Q!invariant()!, and so it is \trusted.
\end{iitemize}
