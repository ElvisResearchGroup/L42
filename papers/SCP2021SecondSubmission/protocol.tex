...
Note that a method performing a field \emph{update} of a \Q@capsule@ field (instead of a field access) is not a capsule mutator, but just a normal method calling a field update.
Capsule mutators handle the more subtile case where the fields of the object with invariant are not updated but the invariant may still be broken because of mutations deep in the ROG.
