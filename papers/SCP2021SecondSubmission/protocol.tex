...
Here we present a novel kind of \Q!Rep! field\footnote{As for the other kinds of \Q@capsule@ fields, our new kind is also just 
a private \Q@mut@ fields with extra restrictions.} (which can coexist with other kinds of \Q!rep! fields),
enforcing the following key property:
the ROG of a \Q!rep! field $o.f$ can only be mutated under the control of a \Q@mut@ method of $o$, and during such mutation, $o$ itself cannot be seen.
This is similar to owner-as-modifier~\cite{Dietl05universes:lightweight,10.1007/978-3-540-92188-2_4}, 
where we could consider an object to be the `owner' of all the mutable objects in the ROG of its \Q!rep! fields, but with the extra restriction that the owner is unobservable during mutation of this ROG.

More preciselly, if a reference to an object in the ROG of a \Q!rep! field $o.f$ is involved in execution as \Q!mut!, then:
...
Note that a method performing a field \emph{update} of a \Q@rep@ field (instead of a field access) is not a rep mutator, but just a normal method performing a field update.
Rep mutators handle the more subtle case where the fields of an object with invariant are not updated, but a mutation deep within their ROG may potentially break the invariant.
...
\noindent The above rules ensure that rep mutators control the mutation of the ROG of \Q!rep! fields, and 
 ensures our points (1), (2), and (3):
$o$ will not be in the ROG of $o.f$ and
only a rep mutator on $o$ can see $o.f$ as \Q!mut!; this means that the only way to mutate the ROG of $o.f$ is through such methods. 
%When a rep mutator is not executing, no object in the ROG of $o.f$ can be seen as \Q!mut! or \Q!capsule!. 
...
Thus we can be sure that the ROG of $o.f$ will only be mutated within a rep mutator, and only after the single use of $o$ to access $o.f$.
...
%\IODel{In order to allow complex mutations of objects with invariants we, introduce the notion of \emph{rep mutator}}. A \emph{rep mutator} can perform an arbitrarily complex mutation of the ROG of a \Q!capsule! field. We use TMs to ensure that the object containing the \Q!capsule! field is not usable whilst the field's ROG is mutated, and its invariant is checked immediately afterwards. 
