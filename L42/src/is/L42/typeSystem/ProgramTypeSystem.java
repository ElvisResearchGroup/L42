...
public class ProgramTypeSystem {
  ...
  private static void auxTypePlugin(Program p, MWT mwt,J j) {
    ...
    if(!nativeUrl.startsWith("trusted:")){
      ...
      errIf(!ok,mwt.poss(),Err.nativeParameterInvalidKind(!mwt.nativeUrl().isEmpty(),mwt.nativeUrl(),mwt.key(),"imm or class",mh.mdf(),"imm or class"));
      var errs=L(mh.pars().stream().filter(t->!t.mdf().isImm()));
      errIf(!errs.isEmpty(),mwt.poss(),Err.nativeParameterInvalidKind(!mwt.nativeUrl().isEmpty(),mwt.nativeUrl(),"imm",mwt.key(),errs,"imm"));
      return;
      }
