...
public class Full {
  ...
  public static final class L implements LL, Leaf, Half.E, Visitable<L> {
    @Override
    ...
    public List<LDom> dom() {
      return ms.stream().map(m -> m.key()).collect(Collectors.toList());
    }

    @Override
    public L c(C c) {
      return (L) ms.stream().filter(m -> m.key().equals(c)).findFirst().get()._e();
    }
