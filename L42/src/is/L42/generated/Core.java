// Generated by delombok at Sun Sep 08 21:03:56 PETT 2019
...
public class Core {
  ...
  public static final class L implements LL, Leaf, Half.Leaf, Visitable<L> {
    ...
    public List<LDom> dom() {
      return L(Stream.concat(mwts.stream().map(m -> (LDom) m.key()), ncs.stream().map(m -> (LDom) m.key())));
    }

    @Override
    public L c(C c) {
      return ncs.stream().filter(m -> m.key().equals(c)).reduce(toOneOr(() -> bug())).get().l();
    }
