...
public class L42£Map<K,T> extends AbstractStructuredCache<L42£Map<K,T>,L42£Map<K,T>>
implements L42Cachable<L42£Map<K,T>>{
  LinkedHashMap<K,T> mapImms=null;
  LinkedHashMap<K,T> mapMuts=null;
  K[] keys=null;
  T[] vals=null;
  L42£Map<K,T> norm=null;
  Supplier<L42Cache<K>> kCache;
  Supplier<L42Cache<T>> vCache;
  public boolean isEmpty(){
    return (mapImms==null || mapImms.isEmpty()) && (mapMuts==null || mapMuts.isEmpty());
    }
  L42£Map(Supplier<L42Cache<K>> kCache,Supplier<L42Cache<T>> vCache){this.kCache=kCache;this.vCache=vCache;}
...
/*  @Override public boolean eq(L42£Map<?,?> other){
  ...
  @SuppressWarnings("unchecked")
  public static final Class<L42£Map<?,?>> _class=(Class<L42£Map<?,?>>)(Object)L42£Map.class;
  @Override public L42£Map<K,T> myCache(){return this;}
  //static{L42CacheMap.addCachableType_synchronized(_class,myCache);}
  @Override public L42£Map<K,T> newInstance(){throw unreachable();}

  @Override public boolean isNorm(L42£Map<K, T> t) {return norm!=null;}
  @Override public L42£Map<K, T> getMyNorm(L42£Map<K, T> t) {return norm;}
  @Override public void setMyNorm(L42£Map<K, T> t, L42£Map<K, T> norm){this.norm=norm;}
  
  @Override public void setNorm(L42£Map<K, T> norm){this.norm=norm;}//from cachable
  @Override public L42£Map<K, T> myNorm(){return norm;}//from cachable
  
  @Override public Object[] f(L42£Map<K, T> t){
    loadIteration();
    return vals;
    }

  @Override public Object f(L42£Map<K, T> t, int i) {
    loadIteration();
    return vals[i];
    }

  @Override public void f(L42£Map<K, T> t, Object o, int i) {
    @SuppressWarnings("unchecked")
    T v=(T)o;
    loadIteration();
  ...
  @Override public int fn(L42£Map<K, T> t){return mapMuts.size();}

  @Override public L42Cache<?> rawFieldCache(int i){return vCache.get();}

  @Override public Object typename(){return TrustedKind.HMap;}

  @Override protected L42£Map<K, T> _fields(L42£Map<K, T> t){return this;}

  @Override protected Object f(L42£Map<K, T> t, int i, L42£Map<K, T> _fields){return f(t,i);}
  
  @Override protected void setF(L42£Map<K, T> t, int i, Object o, L42£Map<K, T> _fields){f(t,o,i);}

  @Override protected L42£Map<K, T> newInstance(L42£Map<K, T> t) {
    return new L42£Map<K, T>(kCache,vCache);
    }
