...
public class L42£MutMap<K,T> extends AbstractStructuredCache<L42£MutMap<K,T>,L42£MutMap<K,T>>
implements L42Cachable<L42£MutMap<K,T>>{
  LinkedHashMap<K,T> mapImms=null;
  LinkedHashMap<K,T> mapMuts=null;
  K[] keys=null;
  T[] vals=null;
  L42£MutMap<K,T> norm=null;
  Supplier<L42Cache<K>> kCache;
  Supplier<L42Cache<T>> vCache;
  public boolean isEmpty(){
    return (mapImms==null || mapImms.isEmpty()) && (mapMuts==null || mapMuts.isEmpty());
    }
  L42£MutMap(Supplier<L42Cache<K>> kCache,Supplier<L42Cache<T>> vCache){this.kCache=kCache;this.vCache=vCache;}
...
/*  @Override public boolean eq(L42£Map<?,?> other){
  ...
  @Override public L42£MutMap<K,T> newInstance(){throw unreachable();}

  @Override public boolean isNorm(L42£MutMap<K, T> t) {return norm!=null;}
  @Override public L42£MutMap<K, T> getMyNorm(L42£MutMap<K, T> t) {return norm;}
  @Override public void setMyNorm(L42£MutMap<K, T> t, L42£MutMap<K, T> norm){this.norm=norm;}
  
  @Override public void setNorm(L42£MutMap<K, T> norm){this.norm=norm;}//from cachable
  @Override public L42£MutMap<K, T> myNorm(){return norm;}//from cachable
  
  @Override public Object[] f(L42£MutMap<K, T> t){
    loadIteration();
    return vals;
    }

  @Override public Object f(L42£MutMap<K, T> t, int i) {
    loadIteration();
    if(i%2==0){return keys[i/2];}
    return vals[i/2];
    }

  @Override public void f(L42£MutMap<K, T> t, Object o, int i) {
    @SuppressWarnings("unchecked")
    T v=(T)o;
    loadIteration();
    assert i%2!=0;
  ...
  @Override public int fn(L42£MutMap<K, T> t){return mapMuts.size()*2;}
  ...
  @Override public Object typename(){throw unreachable();}//return TrustedKind.HMap;}

  @Override protected L42£MutMap<K, T> _fields(L42£MutMap<K, T> t){return this;}

  @Override protected Object f(L42£MutMap<K, T> t, int i, L42£MutMap<K, T> _fields){return f(t,i);}
  
  @Override protected void setF(L42£MutMap<K, T> t, int i, Object o, L42£MutMap<K, T> _fields){f(t,o,i);}

  @Override protected L42£MutMap<K, T> newInstance(L42£MutMap<K, T> t) {
    return new L42£MutMap<K, T>(kCache,vCache);
    }
