...
//          ms1..msk={ms|p(Pi)(ms) is defined}
  ...
  for(ClassB.Member mij:cb0.getMs()){mij.match(
    nc->null,
    mi->add(true,ms1k,mi.getS(),mij),
    mt->add(true,ms1k,mt.getMs(),mij)
    );}
  for(ClassB cbi:cb1n){
    for(ClassB.Member mij:cbi.getMs()){mij.match(
      nc->null,
...
private Void add(boolean isFirst,HashMap<Ast.MethodSelector,List<ClassB.Member>> ms1k,Ast.MethodSelector ms,ClassB.Member m){
...
private boolean exactly1NoRefine(List<Member> memsAll) {
  int count=0;
