...
public interface ExpCore {
	...
	final class ClassB implements ExpCore, Ast.Atom, HasPos {
		...
		public ClassB(Doc doc1, Doc doc2, boolean isInterface, List<Path> supertypes, List<Member> ms, Position p, ast.Util.CachedStage stage) {
			...
			this.stage = stage;
			this.p = p;
	      if(p==Position.noInfo && !ms.isEmpty() &&ms.size()!=5){
        assert true;
        }
			assert stage != null;
			isConsistent();
		...
		public boolean isConsistent() {
                                    Position pOut=this.p;
                                    for(Member m:this.getMs()){
                                      if(m instanceof NestedClass){
                                        NestedClass nc=(NestedClass)m;
                                        nc.getInner().accept(new CloneVisitor(){
                                      @Override public ExpCore visit(ClassB cb){
                                        if( pOut==cb.p){
                                        System.out.println("TempClass With corrupted positions");
                                        }
