...
\\\production{%
R}      {$Cs_1 \eq{T_1}\ldots Cs_n\eq{T_n}$}                    {redirect map}
\end{bnf}
\end{minipage}
...
DO WE NEED TO ADD NESTEDS HERE?
...
The intuition behind redirectSet is that if the signature of a nested class mention another nested class, they must be reidirected together.
...
For $p'\op{bestRedirection}{}$ to be well defined, we need to check that $p\op{redirectable}{Csz}$
This is again a check local to the $LV$ (the top of the program) and is also easly computable.
...
Finally,  $p\op{bestRedirection}{R}$ 
is the centerpiece of our formal framework.
Given an $p$ and an $R$ that are valid input for redirection as defined above, we can denote the best complete map, mapping any elemnt of $Csz$ into a suitable type in $p$.
We ...
can be re redirectable in order
...
\defy{LV({\emptyset})}{LV}%
\defy{\lib{\_}{\_, C\eq{L_0}, \_}{\_}(\Cs{C}{\s{C}})}{L_0(Cs) }%\text{ where } L = \lib{\_}{\_, C\eq{L_0}, \_}{\_}}%
	\defyc{\text{where } L = a}
\end{defye}
