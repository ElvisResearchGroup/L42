...
We define the following usual functional-notations so that $\Mz$ can be treated as a function from $\Id$ to $M$:
...
\begin{defs}
	...
	\defi{\clit{K}{\Tz}{\Mz}[\Cs{C}{Cs}=E]}{\clit{K}{\Tz}{\Mz[C=\Mz(C)[Cs=E]]}}{}
...
When we `enter' (e.g. by the {\sc L-ENTER} compilation rule) a nested class, we will have a program of the form $p = \prog{\cd{C'}{L(C)}, \tld{C}{L}}{DVs}$, and so $p(\This{0}) = L(C) = p(\This{1}{C})$. Similarly, a nested class of $L(C)$ can be entered and so on. This causes a class to have multiple-valid names, to handle this we define an operation that returns the shortest valid one:
...
\begin{defs}
	...
	\defi{\mini{p}{\ncd{C}{LV}}}{\mini{(\tld{C}{LV}, p)}{LV}}{propagate to a nested class definition}
\end{defs}
We will assume that \mini{p}{LV} and \mini{p}{\meth{T}{m}{\Txs}{e\q}} simply propagate to \mini{p}{\ldots} on all their sub-forms.
...
\begin{itemize}
	\item \Rulename{Top}: this rule compiles the first non-flattened top-level declaration. It evaluates it using a well-typed subset ($\DVz$) of the previously compiled ($Ds$) declarations. See Section \ref{?} for the definition of the typing judgement $\DVz\vdash\textbf{Ok}$ \IOComm{Why do we use an arbitrary well-typed subset and not a maximal one, or even just all of them?}
	...
	\item \Rulename{Sum}: this rule first minimizes the operands to the sum, and then reduces to the oprepriate value. See Section \ref{?} for the definition of $\summ{LV_1}{LV_2}=LV$.
	\item \Rulename{Redirect}: this is the start of the reduction for our novel redirect operator. It proceeds in steps: \IOComm{What is $R/\id$ supposed to be?}
	\begin{itemize}
		\item It first creates a new program $p'$, so that \This{0} refers to the (minimized) LHS of the redirect. 
		\item We then compute $\Csz$ which is the \emph{redirect set}: the set of nested class names that are transitively reachable from the domain of $R$ (the classes that we are `redirecting'). 
		\item We verify that these classes are all suitable for redirection (they have to exist, cant have nested classes that aren't going to be redirected, and they cant be private).
		\item We then compute a complete mapping, $R'$, which is the `best'  redirection that extends from $R$. 
		\item Finally, we apply the mapping to $LV'$, with the classes we are redirecting away ($Csz$) removed.
	\end{itemize}
\end{itemize}
