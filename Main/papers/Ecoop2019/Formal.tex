...
\subsection{Preferential sum and examples of sum and redirect working together}
...
have to be adapted so that when code is moved around in differed depths of nesting the 
refereed classes stay the same.
To this aim we define a concept of program $p$, as a representation of 
the code as seen from a certain point inside of the source code.
Each of the $\textit{DV}_0\ldots\textit{DV}_n$
represents the outer nested level $0..n$, while
the \textit{DLs} component represent the program top level, that is 
...
C={D={E=t}}
\end{lstlisting}
the flattened version for \Q@C@ will be 
\Q@C={D={E={ B={()}   This3.A m(This0.B b)}}}@, where the path
\Q@This1.A@ is now \Q@This3.A@ while the path \Q@This0.B@ stays the same.
The program $p$ in the observation point \Q@D=t@ is
\begin{lstlisting}
A={()}
t={ B={()}   This1.A m(This0.B b)}
C={D={E=t}};
