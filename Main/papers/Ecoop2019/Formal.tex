...
\begin{itemize}
	...
	\item Members ($M$): a class member is either a method declaration or a nested class declaration. A method declaration can optionally be abstract (if it's $e?$ is empty) or static (if it has the \Q{static} keyword). A (nested) class declaration can be \Q{private} and defines a class with name $C$ to be the flattened result of a code expression $E$.
	...
	\item Redirect Map ($R$): A redirect map is a sequence of mappings from $Cs$s to $T$s. A $Cs$ indicates the name of a nested class within the LHS of the redirect operator, whereas the $T$ is the target of the redirect. For example $E<C1.C2 = T>$ refers to the nested class named $C2$ that is in the class named in $C1$ in the result of $E$.
	...
	\begin{itemize}
		\item Not all the enclosing code literals may have names, for example the LHS of a $\summ{}{}$ operation is anonymous, so one would need to use $This_0$ to refer to it. \IOComm{Is this true???}
...
\item An $e.x$ or $\Q{new} T(es)$ expression not within the main expression (i.e. inside an $L$) must have $e = \Q{this}$ and $T = \Q{This}_0$. This ensures that fields and constructors are instance-private. \IOComm{But not in the main expression?} \IOComm{Mention that the enclosing $L$ must have appropriate fields? Or should this be a type rule instead? Actually can't we just make this whole well formedness rule a type rule?}
...
When we `enter' (e.g. by the {\sc L-ENTER} compilation rule) a nested class, we will have a program of the form $p = \prog{\cd{C'}{L(C)}, \cd{C}{L}}{DVs}$, and so $p(\This{0}) = L(C) = p(\This{1}{C})$. Similarly, a nested class of $L(C)$ can be entered and so on. This causes a class to have multiple-valid names, to handle this we define an operation that returns the shortest valid one:

\begin{defs}
	\defi{p\op{min}{\This{n+1}{\id_n}{Cs}}}{p\op{min}{\This{n}{Cs}}}{text}
	\defic{\text{where } p = \prog{\cd{id_0}{L_0},...,\cd{id_n}{L_n},\_}{Ds}}{more text}
	\defi{p\op{min}{T}}{T,\text{otherwise}}{}
\end{defs}
\IOComm{Restrict when $p\op{min}{T}$ is defined?}

Note how based on our definitions above, the meaning of a type $T$ depends on our current context, or program. We will define an operator $T\op{from}{T', j}=T''$ \IOComm{Marco can you explain `what' from does? (not how it does it), in particular, what does the $j$ mean?}
