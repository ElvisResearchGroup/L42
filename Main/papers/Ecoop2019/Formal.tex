...
Here we use the metavariable $x$, $t$, $C$ and $m$ as identifiers for variables/fields, traits, classes and methods (respectively). Where an identifier is a $C$ (a class name) if and only if it starts with a capital letter. In addition we will use the metavariables $n$ and $i$ to refer to natural numbers, and $l$ to refer to memory locations. We will use the notation $Xs$, where $X$ is any metavariable, to refer to a (possibly empty) sequence of $X$s. Similarly $Xz$ will refer to a (possibly empty) \emph{set} of $X$s. \IOComm{Explain this $Xz$ thing in more detail?} Then notation $X?$ represents an $Xs$ with at most one element. Here is the grammar for our language:
...
K}      {Txs \mmid{} \Q{abstract}}                                          {state}\\\production{%
...
v}      {\Q{new} T\rp{ls}}                                                 {value}\\\production{%
LV}      {\ldots}{}                                                
\\\hline
\end{bnf}
\end{minipage}
...
\begin{itemize}
	...
	\item Code Literals ($L$): a code literal is like a class declaration in Java-like languages, however they do not provide a name for the class. A code-literal either represents an \Q{interface}, or a \emph{concrete} class (if it has no \Q{interface} keyword). For ease of explanation we will use the term `class' to refer to either an interface or concrete class. The $Tz$ at the start of the code literal lists all the interfaces that are implemented, while the $Mz$ are the classes methods. Concrete classes additionally have a $K$ denoting its state.
	...
	\item State ($K$): \IOComm{Rename this to $S$?} is either the keyword \Q{abstract} represents a class whose fields are unknown, or a list of field declarations (Type and field names) ($Tx$). A class with \Q{abstract} for its state cannot be the receiver of a \Q{new}, unlike a class with an empty $Txs$ for its state.
	...
	\item Redirect Map ($R$): A redirect map is a sequence of mappings from $Cs$s to $T$s. A $Cs$ indicates the name of a nested class within the LHS of the redirect operator, wheras the $T$ is the target of the redirect. For example $E<C1.C2 = T>$ refers to the nested class named $C2$ that is in the class named in $C1$ in the result of $E$.
	...
	\begin{itemize}
		\item Not all the enclosing code literals may have names, for example in the LHS of a $<+$ operation is anonymous, so one would need to use $This_0$ to refer to it. \IOComm{Is this true???}
	...
	\end{itemize}
	A major consequence of this representation is that two $Ts$ may refer to the same class even if they are not syntactically identical, in particular $This0$ and $This1.C$ are the same if the immediatley enclosing code literal is the RHS for the declaration of $C$.
	...
	\item Values ($v$): these represent fully evaluated runtime expressions ($e$s) which will be stored in memory. This corresponds to a fully reduce \Q{new} expression, here the $T$ will identify a concrete class, and the $ls$ will be (locations) of the field values. \IOComm{Why do we need memory? If we don't have mutation or cyclic structures, can't we just have $v ::= \Q{new} T(vs)$ and drop $l$?}
	\item Flattened Code Literals ($LV$): these are just like normal code literals except that all of its nested class declarations have an $LV$ on their RHS (ass opposed to an un-flattened $E$).
\end{itemize}
