...
A `valid' type is one that can be used as a target in a redirect map. It must be already fully compiled and typed \IOComm{Why do we need it to be typed? Wouldn't compiled (i.e. an $LV$) be sufficient for redirect to make sense?}, and cannot refer to a private class, unless it is ... .
...
\defic{}{check that the targets of $R'$ are reasonable}
\end{defs}

\IOComm{That last line makes no sense... (it was done just to make the proof easier and limit the number of definitions), but in reality all we need to check is that the $T$s in the range of $R'$ are well-typed (and not private?)}
...
\section{A computable \opName{bestRedirectMap}: \opName{choseRedirectMap}}
