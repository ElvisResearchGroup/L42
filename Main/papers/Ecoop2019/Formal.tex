...
Here we use the metavariables $x$, $t$, $C$ and $m$ as identifiers for variables/fields, traits, classes and methods (respectively). 
In the code examples, we will represent $C$ with uppercase identifiers.
%Where an identifier is a $C$ (a class name) if and only if it starts with a capital letter.
Metavariables $n$ and $i$ refers to natural numbers.
We will use the notation $Xs$, where $X$ is any metavariable,
to refer to a (possibly empty) sequence of $X$s.
Similarly $Xz$ will refer to a (possibly empty) \emph{set} of $X$s.
%\IOComm{Explain this $Xz$ thing in more detail?}
 Then notation $X?$ represents an $Xs$ with at most one element. Here is the grammar for our language:
...
As it also happens in Featherweight Java (FJ), 
in our language a main expression $e$ is reduced
under a sequence of top level declarations $Ds$.

Here we expain in more detail our syntax:
%\IOComm{Why is there lots of space here?}
\begin{itemize}
	\item Expressions ($e$): these are similar to FJ ones:
variable $x$, instance method calls 
(where the receiver is an $e$),
static method calls (where the receiver is a $T$),
field accesses and new expressions.
	\item Code Literals ($L$): a code literal is like a class declaration in Java-like languages, however they do not provide a name for the class. The $Tz$ component contains all the interfaces that are implemented,
 while $Mz$ contains all the members.
 %$K$ specifies the kind of class (and its fields, if any).
	\item Kind ($K$): is either the keyword \Q{interface}, representing a class with no direct instances, \Q{abstract} representing a class whose fields are unknown, or a list of field declarations, composed by Type and field names $Tx$; this represents a \emph{concrete} class. For ease of explanation we will use the term `class' to refer to any of these three kind of classes, and the term \emph{proper} class to refer to one that is not an \Q{interface}. The \Q@new@ operator can be used only on concrete classes. 

	\item Members ($M$): a class member is either a method declaration or a nested class declaration.
A method declaration can optionally be abstract (if it's $e?$ is empty) or static (if it has the \Q{static} keyword).
A (nested) class declaration can be \Q{private} and defines a class with name $C$ to be the flattened result of a code expression $E$.
	\item Code Expressions ($E$): are the main interesting part of our language, they are either code-literals, references to a trait, a sum operation, or a redirect operation. Such expressions will be flattened (at compile time) to a Code Literal Value $LV$;
these are just like normal code literals except that all of their nested class declarations have an $LV$ on their RHS, as opposed to an $E$.
	\item Redirect Map ($R$): A redirect map is a sequence of mappings from $Cs$s to $T$s. A $Cs$ indicates the name of a nested class within the LHS of the redirect operator, whereas the $T$ is the target of the redirect. For example $E<C1.C2 = T>$ refers to the nested class named $C2$ inside the nested class named $C1$ in the code literal resulting from $E$.
	\item Types ($T$): a type is a path of the from \This{n}{$Cs$}. It identifies the nested class $Cs$ in the $n^\text{th}$ enclosing code literal ($L$).
That is, in our language types are always local paths; this allows code literals to refer to the nested classes inside of them, while moving traits around transparently preserve those references.
\footnote{This is different with respect to what happens in Java and most other languages, where
types are just paths from top level; scope resolution
allows to write a local path that is translated in the top level path before compilation.}
%
%to 
%This form is  We need this form, as opposed to a more conventional alternative where the $Cs$s would start from top level, for several reasons:
%	\begin{itemize}
%		\item Not all the enclosing code literals may have names, for example the LHS of a $<+$ operation is anonymous, so one would need to use $This_0$ to refer to it. \IOComm{Is this true???}
%		\item It allows for code to refer to classes that will be redirected away, for example in ${ C = {}; This0.C m()}<C = ...>$, $C$ will no exist in the flattened program, so relative to the top-level of the program, it has no name. \IOComm{Is this a bad justification}
%	\end{itemize}
	A major consequence of this representation is that two $Ts$ may refer to the same class even if they are not syntactically identical, in particular $This0$ and $This1.C$ are the same if the immediately enclosing code literal is the RHS for the declaration of $C$.
	...
	\item Identifiers ($id$) are names for classes or traits.
% If their concrete form starts with an upper-case letter they name classes, otherwise traits.
	\item Values ($v$): these represent fully evaluated runtime expressions $e$. This corresponds to a fully reduced \Q{new} expression, here the $T$ will identify a concrete class, and the $vs$ will be the field values.
...
\item 
Expressions $e.x$ and $\Q{new}\ T(es)$
inside method bodies
%An $e.x$ or $\Q{new}\ T(es)$ expression not within the main expression (i.e. inside an $L$)
must have $e = \Q{this}$ and $T = \Q{This}_0$. This ensures that fields and constructors are instance-private. Note that
there is not restriction on the shape of
$e.x$ and $\Q{new}\ T(es)$ in the main expression.
 \IOComm{Mention that the enclosing $L$ must have appropriate fields? Or should this be a type rule instead? Actually can't we just make this whole well formedness rule a type rule?}
...
\begin{bnf}
	\hline
	\production{%
		p} {Ds\Q{;} DVz} {program}\\\production{%
		DV}      {id\eq{}LV} {evaluated declaration}
	\\\hline
\end{bnf}
...
In order to type-check and flatten our language we need to be able to lookup classes and traits, this is complicated since class names are represented in the form \This{n}{$Cs$}, which has a different meaning in different places. To this aim we define a program $p$, which will be be used to keep track of the current state and `location' of operations.
The program is of the form $Ds\Q{;} DVz$, where the $DVz$ represents all the (fully evaluated) top-level declarations (classes or traits). The $Ds$ is a stack of nested classes, where the first $D$ (the top of the stack) is the declaration for \This{0}. The program will only contain top-level declarations that have already been evaluated ($DV$), preventing references to non-flattened traits. \IOComm{Justify why we are doing this?}

We define the following obvious notations to extract the members of a library-literal:

\begin{defs}
	\defi{\fop{members}{\lib{\_}{\_}{Mz}}}{Mz}{get the members from a code literal}
	\defi{L(m)}{\fop{members}{L}(m)}{get a method}
	\defiSep
	\defi{L(\Empty)}{L}{get a (transitively) nested class declaration}
	\defi{L(Cs\Q{.}C)}{\fop{members}{L(Cs)}(C)}{}
	\defiSep
	\defi{\dom{L}}{\dom{\fop{members}{L}}}{get the names of directly nested members}
	\defi{\mdom{L}}{\{m \in \dom{L}\}}{get the names of all methods}
	\defiSep
	\defi{L[\Empty=E]}{E}{update nested class}
	\defi{\lib{K}{Tz}{Mz}[C\Q@.@Cs=E]}{\lib{K}{Tz}{Mz[C=Mz(C)[Cs=E]]}}{}
	\defi{(Mz,\nc{C}{\_})[C=E]}{Mz,\nc{C}{E}}{update member}
	\defiSep	
\end{defs}
...
\begin{defs}
	\defiSep
	\defi{(\prog{}{\_,\cd{C}{L},\_})(\This{0}{C}{Cs})}{L(Cs)}{top level lookup}
	\defi{(\cd{\_}{L}, \_)(\This{0}{Cs})}{L(Cs)}{nested class lookup}
	\defi{(\_, p)(\This{n+1}{Cs})}{p(\This{n}{Cs})}{outer lookup}
	\defiSep
\end{defs}

Note that the notation $\cd{C}{L}, p$ is equivalent to $\prog{\cd{C}{L},Ds}{DVs}$ where $p = \prog{Ds}{Dvs}$. It `pushes' the declaration $\cd{C}{L}$ to the top of the program stack. 
