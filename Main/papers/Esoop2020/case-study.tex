...
This aligns the code with L42, which also performs such checks at runtime.}: that the invariant of \Q@SafeMovable@ was not known to hold at the end of its constructor and \Q@dispatch@ method. Thus, like our system, Spec\# checks the invariant
...
While the same numbers of checks are performed, we do not have the same guarantee provided by our approach:  Spec\#/Boogie does not soundly handle the effect of I/O over non determinism, thus 
...
This is a generalization of the example proposed by[]
