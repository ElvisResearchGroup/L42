...
Our example shows that the restrictions of TMs are flexible enough to encode interactive GUI programs, where widgets may circularly reference other widgets.
...
Both our L42 and Spec\# code required us to use the box pattern for our \Q@SafeMovable@, due to the circular object graph caused by the \Q@Action@s of \Q@Button@s needing to change their enclosing \Q@SafeMovable@'s position. In Appendices~\ref{s:MoreCaseStudies} and~\ref{s:patterns}, we show how the box pattern can also be used to pass the state of invalid objects around, and batch together complex mutations and multiple field updates, with only a single invariant check. Appendix~\ref{s:patterns} also shows a `transformer' pattern, were we can allow the children of \Q!Widgets! to be mutated by arbitrary code, albeit with restrictions.
