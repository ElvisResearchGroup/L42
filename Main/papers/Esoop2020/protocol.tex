...
To ensure that \Q!invariant()! methods do not expose a potentially broken \Q!this! to other objects, we require that all occurrences of \Q!this!\footnote{Some languages allows the \Q@this@ receiver to be implicit. For clarity in this work we require \Q@this@ to be always used explicit.} 
%(as a sub-expression) 
in the body of \Q!invariant()! methods are the receiver of a field access (of form \Q!this.f!) of an \Q!imm!/\Q!capsule! field, or the receiver of a method call (of form \Q!this.m(..)!) of a final (non-virtual) method that in turn satisfies these restrictions. No other uses of \Q!this! is allowed, such as as the RHS of a variable declaration, or an argument to a method.
...
but the \Q@invariant()@ method is unable to access the list's nodes, since they contains \Q@mut@ references to shared/unencapsulated objects.
...
as being deterministic and not mutating existing memory. This holds because % ISAAC: define pure earlier on in the paper so we don't have to say "deterministic and does not mutate pre-existing memory" all the time? %Marco: I checked, this is the first point where we use determinism in this sense.
...
\footnote{
In L42, our new kind of \Q!capsule! can coexists with other kinds of \Q!capsule! field, such as those designed for safe parallelism~\cite{GordonEtAl12,clebsch2015deny,GIANNINI2019145}.}



% ISAAC: I think this paragraph should go somewhere else...
Our \Q!capsule! fields enforce the following key properties: if a reference to an object in ROG of a capsule field $o.f$ is involved in execution as \Q!mut!, then:
(1) no reference to $o$ is involved in execution, (2) a call to a \Q!mut! method for $o$ is above the stack frame.
...
This means that when the ROG of $o.f$ is being mutated, $o$ itself cannot be seen, and all such mutations are under the control of a \Q!mut! method of $o$.

To show how our \Q@capsule@ fields ensure these properties, we first define some terminology:
\Q@x.f@ is a \emph{field access}, \Q@x.f=e@ is a \emph {field update}\footnote{Thus a field update \Q@x.f=e@ should not be read as field access followed by an assignment.}
...
\Q!capsule!.\footnote{This is safe since a \Q@capsule@ receiver object will then be unreachable, so do not need to preserve its invariant.}
\end{itemize}
...
L42 constructors directly initialize all fields using their parameters, and L42 does not provide traditional subclassing.
This works naturally with with our invariant protocol.
We can support traditional constructors as in Pony and Gordon \etal's language, by allowing \Q!this! to be used only as the receiver of field initialisations (in constructors bodies). 
