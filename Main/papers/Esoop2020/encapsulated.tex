...
Capsule fields allow expressing invariants over mutable object graphs.
...
class ShippingList {
  ...
  mut method Void addItem(Item item) {
    ...
    if (!this.invariant()){throw Error(...);}}}//injected check
\end{lstlisting}
Our protocol injects calls to \Q@invariant()@ at the end of the constructor and the \Q@addItem(item)@ method.
...
would break our protocol: if capsule mutators could declare a \Q@mut@ return type the following would be accepted:
\begin{lstlisting}
