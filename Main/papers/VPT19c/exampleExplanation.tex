...
Our approach, as presented in this \IO{extended abstract}, only guarantees that \IO{code resulting from metaprogramming} follows its own contracts, it does
not \MS{statically} ensure what \IO{those contracts may be}. As future work, we are investigating how \IO{the resulting contracts can be ensured to have a particular meaning} \MS{or form}.
...
/@ensures(result.$\mathit{methName}$.ensures ==>\ $\mathit{predicate}$)/
\\*
would \IO{use a static verifier to ensure that $\mathit{methName}$'s /@ensures/ clause logically implies $\mathit{predicate}$.}
With \IO{these two features} we could annotate the method /generate/ in line 32 \IO{above} as:
\begin{lstlisting}
@requires(exp>0)
