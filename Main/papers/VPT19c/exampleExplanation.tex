...
\MS{In this short paper we just require them to be syntactically identical.} Relaxing this constraint is an important future work.
...
\MS{Executing /compose(base,even)/ or /compose(base,odd)/ will succeed this test: the contract of /base.pow()/
is the same of /even.$\_$pow()/ and /even.$\_$pow()/; and the same holds for /exp()/.}
...
\MS{Following general terminology in software verficiation, we call \emph{correct} a trait whose methods respect their contracts.}
\MS{Informally, our result is that starting from a set
of well typed and correct traits, 
the code resulting from arbitrary many steps of trait composition will be correct and well typed too.
In this way, the programmer need only to provide correct bulding blocks as traits;
code generated by metaprogramming can be integrated with a correct program without the need of using expensive theorem provers or manual verification.
