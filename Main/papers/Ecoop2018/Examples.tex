...
\item \Rulename{sum} either preserve or reduces the number of
wrong literals:
the core of the proof is to show the sum of two well typed literals produces a well typed one.
A code literal is well typed (\Rulename{l-ok}) if implements all the methods of the super interfaces (true since true for both arguments) and all their bodies are correct.
