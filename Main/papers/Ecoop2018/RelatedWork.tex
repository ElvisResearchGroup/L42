...
\subsection{Disconnecting Inheritance and Subtyping}
...
\end{lstlisting}
The syntax \lstinline{.B} denote a relative path, that is, the 
class \lstinline{B} in scope.
In Java and Scala declaring in a subclass a nested class with the same name of a nested of the superclass has only the effect of hiding parent's declaration.
