...
Consider the example from Section~\ref{sec:intro}:
\saveSpace
\begin{lstlisting}
...
literal associated with the class is `\Q@{ method int ma(){return Utils.m(this);}}@' and it contains the members.
...
Note that also this second attempt would not work:
\saveSpace
\begin{lstlisting}
...
Interfaces are the only way to obtain subtyping;
