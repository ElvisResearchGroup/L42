...
class PointAlgebra=Use pointSum,pointMul, {//not 42 code
    ...
    method int x() x//repetitive code
    method int y() y// in traits terminology, this is all "glue code"
...
\item {\bf Disadvantages:} Writing such obvious definitions to close
  the state/fixpoint in the class 
   with the constructors and fields and getter/setters and factories is tedious and error prone.
...
\item a class with no abstract methods is coherent (just like Java
  \Q@Math@, for example). Such classes are only useful for calling static methods.
...
\saveSpace
\begin{lstlisting}
  //Works because resulting class is coherent.
...
we consider \Q@Sum@, \Q@Subtraction@, \Q@Multiplication@ and \Q@Division@.
This give us 16 different possible combinations.
We coded this example in three different styles:
(a) Java7 (= 243 lines), (b) Classless Java (= 226 lines) and (c) \name (= 57 lines).
...
on the other side for space limitations, the code presented in the article
is formatted to be very compacts.
...
\Q@this@.  This solution is better but still not satisfactory, the color from \Q@that@ is ignored.
...
\noindent Consider the simple flavored trait:
\saveSpace\begin{lstlisting}
  flavored:{
...
\noindent In order to merge colored and flavored we use  \Q@super@ to introduce method selectors \Q@_1merge@ and \Q@_2merge@
  ...
  FCPoint:Use
    ...
    pointSum,{
      ...
      method This merge(This that)//merge conflict is solved 
        this._1merge(that)._2merge(that)//by calling the two versions
      }
\end{lstlisting}  \saveSpace\saveSpace
...
Like the previous experiment, we still found that the Java solution was longer (109 lines) than that in \name (50 lines). This is caused by the absence support for the `This` type, where the withers in each of the \Q@CPoint@/\Q@FCpoint@ classes need to be repeated to make sure that the returned type will be correct (the number of members in Java were 27 while 24 (less 3) in \name).

Complex patterns in Java allow to support the `\Q@This@' type and the `\Q@This@' type instantiation but they required a lot of setup code. We started experimenting using those patterns, but it soon became very clear that the resulting code of this approach would have been even larger; albeit without duplicated code.
