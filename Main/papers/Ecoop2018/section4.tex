...
(a) Java7 \emph{(= 243 lines)}, (b) Classless Java \emph{(= 226 lines)} and (c) \name \emph{(= 57 lines)}.
...
Like the previous experiment, we still found that the Java solution was longer (109 lines) than that in \name (50 lines). This is caused by the absence support for the `This' type, where the withers in each of the \Q@CPoint@/\Q@FCpoint@ classes had to be repeated to make sure that the returned type will be correct (the number of members in Java were 27 while 24 (less 3) in \name).
