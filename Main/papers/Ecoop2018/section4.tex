...
class PointAlgebra=Use pointSum,pointMul, {//not 42 code
    ...
    static method This of(int x, int y) new PointAlgebra(x,y)
    }
\end{lstlisting}
...
This gives us 16 different possible combinations.
...
We chose Classless Java since it is a novel approach allowing to encode traits in Java leveraging on 
...
Finally, the comparison with Scala is interesting since abstract type members allows to conveniently encode
`\Q@This@' type.
...
In the detail, we used 6 lines for the initial \Q@Point@ class,
...
Note how the Java code is less modular than the \name code, since \Q@Colored@ and \Q@Flavoured@ do not exists 
as individual concepts.

We also compare with a solution in Scala.
This shows the same level of reuse and modularity of 
...
\Q@Colored@ and \Q@Flavoured@ traits (6*2)
\Q@CPoint@ and \Q@CFPoint@ (12+18).
Note how the major benefit of \name is in the reduction
of the amount of code needed to generate 
