...
To the best of our understanding, visibility based invariants (introduced in 
'Friends Need a Bit More: Maintaining Invariants Over Shared State'[6])
can reason over non encapsulated state, 
but allow for run-time checking is not obvious (and is not implemented), since it would
...
Concurrency is about what types of expressions can be passed to different threads/actors,
not what fields those expressions can have.
In the language of the paper,
reading a capsule field does not always give a capsule expression,
