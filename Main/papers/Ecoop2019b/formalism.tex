...
\begin{itemize}
	\item An implicit program/class table\IODel{.}\IO{, were e use the notation $C.m$ to get the method declaration for $m$, within class $C$, similarly we use $C.f$ to get the field declaration of field $f$.}
...
\begin{figure}
	...
	\inferrule[(mcall)]{{}_{}}{
		...
		\\*\quad C.f=\Kw{capsule}\,\_ \text{ and }\f\, \text{inside}\, C\singleDot\m
		\\*
...
\IO{The failure of a monitor expression, \M{l}{e_1}{e_2},} \IODel{Monitor failures} will be caught by our \textsc{try error} rule, as will any other uncaught monitor failure in $e_1$ or $e_2$.
