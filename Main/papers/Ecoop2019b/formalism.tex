...
\noindent To encode object capabilities and I/O, we assume a special location  $c$ of class \Q@Cap@. This location would refer to an object with methods that behave non-deterministically, such methods would model operations such as file reading. In order to simplify our proof, we assume that:
\SSI\begin{itemize}
...
\indent $\VS(\s, e)$ iff $c\mapsto\Kw{Cap}\{\_\}|e_0\rightarrow^+ \s|e$, for some $e_0$ with:
\begin{iitemize}
