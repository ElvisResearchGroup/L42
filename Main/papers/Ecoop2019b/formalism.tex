...
defining and proving \IO{the correctness of}\IODel{,} such a type system would require a paper
of its own, and indeed many such papers exist in literature%
~\cite{ServettoEtAl13a,ServettoZucca15,GordonEtAl12,clebsch2015deny,JOT:issue_2011_01/article1}.\IOComm{Papers 67 and 22 don't contain proofs!, I'm not sure of the other ones}
...
\end{itemize}
\IOComm{Later on we use the notations $C.m$ and $\sigma(l).f$, should these be defined here?}
...
\begin{itemize}
	...
	\item all methods in the \Q@Cap@ class must require a \Q@mut@ receiver, and will mutate its ROG, \IO{such methods may be non-deterministic, i.e. may have multiple possible reductions,}
...
in this way there is no value associated with \IO{an} \IODel{the} \emph{error}.
Our L42 implementation instead \IODel{models exceptions as throwing an \Q@imm@ value} \IO{allows arbitrary \Q!imm! values to be thrown as exceptions}, formalising exceptions in this way would not cause any interesting variation of our proof.
...
are run time expressions and thus are not present in method bodies, rather they are generated by our reduction rules inside the main expression. Here, $l$ refers to the object being monitored, $e_1$ is the expression which is being monitored, and $e_2$ denotes the evaluation of $l.\invariant$. \IO{$e_1$ will be evaluated to a value, and the $e_2$ will be further evaluated}, \IODel{If, at any point in execution, $\e_2$ is \Q!false!,} \IO{if $e_2$ evaluated to \Q!false!, or an $\mathit{error}$}, then $l$'s invariant failed to hold; such a monitor expression corresponds to the throwing of an unchecked exception.
...
\begin{itemize}
	...
	monitoring new to this work.} \IO{In Appendix~\ref{s:proof}, we show that this property is preserved thanks to strong exception safety.}
\end{itemize}
...
\begin{figure}
	...
	\inferrule[(mcall)]{{}_{}}{
		...
		\IODel{\text{if }\ \exists \f\text{ such that } C.f=\Kw{capsule}\,\_,
		\mdf=\Kw{mut},}
		\\*\IODel{\quad\f\, \text{inside}\, C\singleDot\m
		\text{, and }
		\f\,\text{inside}\, C\singleDot\Kw{invariant}}
		\\*
		
		\IO{\text{if } \mdf=\Kw{mut}\text{ and }\exists \f\text{ such that }}
		\\*\IO{\quad C.f=\Kw{capsule}\,\_ \text{ and }\f\, \text{inside}\, C\singleDot\m}
		\\*
...
For example, in L42 we implement them by statically injecting calls to \Q!invariant! at the end of setters, factory methods and capsule mutators; this works as L42 \IO{follows the uniform access principle, so it} does not have primitive expression forms for field updates and constructors, rather they are uniformly represented as method calls.
...
\IODel{Our \textsc{ctxv} rule evaluates monitor expressions, \M{l}{\e_1}{\e_2}, by first evaluating $\e_1$ and then $\e_2$. If $\e_2$ evaluates to \Q@true@, then the monitor succeeded, and will yield the result of $\e_1$. If however $\e_2$ evaluated to \Q!false!, then the monitor failure}
\IO{Monitor failures} will be caught by our \textsc{try error} rule, as will any other uncaught monitor failure in $e_1$ or $e_2$.
