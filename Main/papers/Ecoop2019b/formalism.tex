...
\begin{itemize}
	...
	\item \IO{Memory, $\sigma : l\rightarrow C\{\Many{v}\}${, is} a finite map from locations, $l$, to annotated tuples, $C\{\Many{v}\}$, representing objects; where $C$ is the class name and $\Many{v}$ are the field values.}
	...
	the expression $\e$ can contain locations and free variables. \IO{The types} of locations \IO{are} encoded in 
...
\begin{itemize}
	...
	\item \IO{Field accesses and updates in methods are of the form $\Kw{this}.f$ or $\Kw{this}.f\equals\e$, respectively.}
	\item \IO{Field accesses and updates in the main expression are of the form $l.f$ or $l.f\equals\e$, respectively.}
	\item \IO{Locations} that are preserved by \Q@try@ \IO{blocks} are
	never monitored, that is, for $\Kw{try}^\sigma\oC\e\cC\ \_$, \IO{if} $\e$ \IO{is of} the form $\ctx[\Kw{M}\oR l;\_;\_\cR]$, \IO{then} $l\notin\sigma$.
\end{itemize}
...
\IO{We define the relation \emph{inside} as follows:}\par
$%\begin{array}{l}
...
\noindent Monitor expressions are added \IO{after} all field updates, \Q@new@ expressions, and calls to capsule mutators.
...
\noindent 
To allow the invariant method to be called on an invalid object, and access fields on such object, we define the set of trusted execution steps \IO{as the} the call to \Q@invariant@ itself, and any field accesses inside its evaluation. \IO{Note that this only applies to single small step reductions, and not the entire evaluation of \Q!invariant!.}
...
\noindent Finally, we define what it means to soundly enforce our invariant protocol: every object \IO{referenced by} any untrusted redex is valid.
