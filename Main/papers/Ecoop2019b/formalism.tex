...
<<<<<<< HEAD
	...
	We use the notation $\s[l.f=v]$ to update a field of \IODel{an object} $l$, \IODel{and} $\s[l.f]$ to access one\IO{, and $\s \setminus l$ to delete $l$}.
...
<<<<<<< HEAD
a variable environment, $\G : x\rightarrow T$. \E encodes the location \IODel{location}\IO{, }\IODel{(}relative to the top-level expression we are typing\IODel{)}\IO{,} where $e$ was found; this is needed so that $l$s can be typed with different type-modifiers when in different positions.
	\item We use \Ss to trivially extract the corresponding \S from a \s.
=======
a variable environment, $\Gamma : x\rightarrow T$. \E encodes the location \IODel{location}\IO{, }\IODel{(}relative to the top-level expression we are typing\IODel{)}\IO{,} where $e$ was found; this is needed so that $l$s can be typed with different type-modifiers when in different positions.
	\item We use $\Sigma^\sigma$ to trivially extract the corresponding $\Sigma$ from a $\sigma$.
>>>>>>> 9b685242d5a2952aa13c0236f51f67ef973602f2
\end{itemize}

\noindent To encode object capabilities and I/O, we assume a special location  $c$ of class \Q@Cap@. This location would refer to an object \IO{with methods that behave non-deterministically, such methods would model operations such as file reading.} \IODel{whose fields model things like the content of files}. In order to simplify our proof, we assume that:
\SSI\begin{itemize}
	\item \Q@Cap@ \IO{has no fields} \IODel{can only have \Q@mut@ fields, and}.
	\item instances of \Q@Cap@ cannot be created with a \Q@new@ expression,
	\item \Q@Cap@'s \Q@invariant@ method is defined to \IO{have a body of `\Q!true!', and} \IODel{return \Q@true@.}
	\item all \IO{other} methods in the \Q@Cap@ class must require a \Q@mut@ receiver\IODel{, and will mutate its ROG,}\IO{;} such methods \IO{will have a} \IODel{may be} non-deterministic \IO{body}, i.e. \IO{calls to them} may have multiple possible reductions\IODel{,} \IO{.}
\end{itemize}
...
\inferrule[(try error)]{{}_{}}{
	\s,\_|\Kw{try}^\s\oC \error\cC\ \Kw{catch}\ \oC e\cC\rightarrow \s|e
}
\quad
\inferrule[(access)]{{}_{}}{
...
They are pretty standard, except for our handling of monitor expressions. \IODel{\NL} Monitor expressions are added after all field updates, \Q@new@ expressions, and calls to capsule mutators.
...
%\loseSpace
<<<<<<< HEAD
\indent $\trusted(\EV,r_l)$ iff\IO{, either:}
\begin{iitemize}
\item[\IO{$\bullet$}] \IODel{either} $r_l=l.\invariant$ and
$\EV=\EV'[\M{l}{v}{\square}]$, \IO{or}
\item[\IO{$\bullet$}] \IODel{or} $r_l=l\singleDot f$ and
$\EV=\EV'[\M{l}{v}{\EV''}]$.
\end{iitemize}
%\loseSpace
...
\indent $\VS(\s, e)$ iff $c\mapsto\Kw{Cap}\{\IODel{\_}\}|e_0\rightarrow^+ \s|e$, for some $e_0$ with:
\begin{iitemize}
\item[\IO{$\bullet$}] $c:\Kw{Cap};\emptyset\IO{; \h} \vdash e_0: \T$, $\M{\_}{\_}{\_} \notin \IO{e_0} \IODel{e}$, and if $l \in e_0$ then $l = c$.
\end{iitemize}
%\loseSpace
...
=======
\indent $\trusted(\ctx_v,r_l)$ iff\IO{, either:}
\SS[0.25]\begin{itemize}
\item[\IO{$\bullet$}] \IODel{either} $r_l=l.\invariant$ and
$\ctx_v=\ctx_v'[\M{l}{v}{\square}]$, \IO{or}
\item[\IO{$\bullet$}] \IODel{or} $r_l=l\singleDot f$ and
