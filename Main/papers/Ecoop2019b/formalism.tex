...
To keep our small step semantics as conventional as possible, we follow Pierce~\cite{pierce2002types} and Featherweight Java~\cite{IgarashiEtAl01}, and assume:\SS
\begin{itemize}
	\item An implicit program/class table; we use the notation $C.m$ to get the method declaration for $m$, within class $C$, similarly we use $C.f$ to get the \IODel{field} declaration of field $f$\IO{, and $C.i$ to get the declaration of the $i$\textsuperscript{th} field}.
	...
	\item A typing relation, $\Sigma;\Gamma\IO{;\E}\vdash\e:T$, where 
...
a variable environment, $\Gamma : x\rightarrow T$. \IO{\E encodes the location location (relative to the top-level expression we are typing) where $e$ was found; this is needed so that $l$s can be typed with different type-modifiers when in different positions.}
...
\begin{figure}
	...
	\caption{Grammar}\label{f:grammar}\SS[2.5]
\end{figure}
...
\begin{itemize}
	...
	\item \IO{Method bodies do not contain any $l$ or $\M{\_}{\_}{\_}$ expressions.}
\end{itemize}
...
\begin{figure}
	...
	\inferrule[(mcall)]{{}_{}}{
		...
		\e'[\Kw{this}\IODel{=}\IO{\coloneqq}l,\x_1\IODel{=}\IO{\coloneqq}v_1,\ldots,x_n\IODel{=}\IO{\coloneqq}v_n]
	}{
		\begin{array}{l}
		\sigma(l)=C\{\_\}\\
...
\indent or
$r_l=l\singleDot f$ and
$\ctx_v=\ctx_v'[\M{l}{v}{\ctx_v''}]$.
%\loseSpace

\noindent \IO{We define a \VS as one that was obtained by any number of reductions from a well typed initial expression and memory, containing no monitors and with only the $c$ memory location available:}\\
\noindent $\mathit{\VS}(\s, e)$ iff $c\mapsto\Kw{Cap}\{\_\}|e_0\rightarrow^+ \sigma|e$, for some $e_0$ with:\\*
\indent $c:\Kw{Cap};\emptyset\vdash e_0: \T$, $\M{\_}{\_}{\_} \notin \e$, and if $l \in e_0$ then $l = c$.
%\loseSpace

\noindent Finally, we define what it means to soundly enforce our invariant protocol: every object referenced by any untrusted redex\IO{, within a \VS,} is valid.
\SS
