...
access any externally mutable state. % Why do you insist on 'whose execution'?
...
%Why Spec\#?
We present a case study showing that our protocol requires a lower annotation burden compared to Spec\#, and performs orders of magnitude less runtime invariant checks compared to the widely used `visible state semantics' protocols of D, Eiffel, and other languages.
...
We also formalise our invariant protocol and prove 
type-modifiers and object-capabilities are sufficient to ensure its soundness.

---------------------

We investigate a protocol where class-invariants are expected to hold for all objects involved in execution, but
they are soundly checked only in a few points. 

Our approach is sound since we ensure that invariants are pure and checked immediately after an invariant violation could have occurred. This allows for stronger and simpler reasoning, as well as less runtime verification overhead, compared to the more common approach of `visible state' semantics where only the invariant of \Q@this@ is required to hold, and
