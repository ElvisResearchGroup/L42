...
\begin{ienumerate}
	...
	\end{itemize}
		...
		\begin{itemize}
			\item[] Clearly the newly created object, $l$, is \mony. This rule does not modify pre-existing memory, introduce pre-existing $l$s into the main expression, nor remove monitors, thus every pre-existing $l'$ is still \valid (due to \thm{Determinism}), or \mony.
		\end{itemize}
