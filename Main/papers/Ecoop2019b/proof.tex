...
\IOBlock{Put this as a lemma, not a well-formedness requirement}{Note that our last well-formedness rule requires 
	...
	\begin{itemize}
	  ...
	  \begin{itemize}
	    ...
	    \item Otherwise it was $\mathit{valid}$ and $\mathit{wellEncapsulated}$:
			\begin{itemize}
				\item If $l\in \mathit{erog}(\sigma,l_1)$\IO{, by $\mathit{wellEncapsulated}$, not $\mathit{mutatatable}{l, \sigma, e}$, but by \thm{Mut Field}, $\mathit{mutatable}(l, \sigma, e)$, a contradiction.}
	  ...
	  thus it} \IO{$l_0$} being $\mathit{OK}$ could not have been effected by this reduction step.
	\end{itemize}
...
\item (\textsc{try error}) $\sigma,\sigma_0|\Kw{try}^\sigma\oC \mathit{error}\cC\ \Kw{catch}\ \oC\e\cC\rightarrow \sigma|\e$:
	...
	By \thm{Strong Exception Safety}, we know that $\sigma_0$ is $\mathit{garbage}$ with respect to $\ctx_v[\e]$. By our \IOBlock{change this! lemma}{well-formedness criteria}, no location inside $\sigma$ could have been \emph{monitored}.
