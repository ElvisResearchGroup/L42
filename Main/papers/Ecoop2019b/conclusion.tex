...
Our invariant protocol performs several orders of magnitude less checks than visible state semantics, and requires much less annotation\IODel{s} 
...
use invariants to encode pre and post condition\IO{s,
a}s done by~\cite{Flanagan06hybridtypes}: where pre and post conditions are encoded as the invariants of the parameter and return types (respectively).
