...
In appendix~\ref{s:formalism} we formalise our invariant protocol and prove it sound. To stay parametric over the various existing type systems which provably enforce the properties we require for our proof (and much more), we do not formalise any specific type system.
...
\MS{the \Q@invariant@ method can only refer to immutable and encapsulated state.}
\MS{This means that while we} can easily verify that a doubly linked list of immutable elements
\MS{is correctly linked up,}
\REV{we can not do the same for a doubly linked lists of mutable elements.}{2}{???} Our approach does not prevent correctly implementing such data structures, but the \Q@invariant@ method would be unable to access the list's nodes, since they would contain \Q@mut@ references to shared objects.
