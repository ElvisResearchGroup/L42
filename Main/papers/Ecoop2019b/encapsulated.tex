...
\noindent Consider managing \IO{the} \IODel{shipping} \IO{shipment} of items, where there is a maximum combined weigh\IO{t:}
\saveSpace
\begin{lstlisting}
class ShippingList {
  capsule Items items;
...
In order to write correct library code in mainstream languages like Java \IO{and} C++, defensive cloning~\cite{Bloch08} is needed.
%\REVComm{
...
If we were to allow \Q@x.items@ to be seen as \Q@mut@\IO{, where} \IODel{if} \Q@x@ is not \Q@this@, then  even if the \Q@ShippingList@ has full control at initialisation time, such control may be lost later, and code unaware of the \Q@ShippingList@ could break it:
\saveSpace
\begin{lstlisting}
...
\end{lstlisting}
\saveSpace
If we allowed a \Q@mut@ return type \IODel{this} \IO{the following} would be accepted:
\saveSpace
\begin{lstlisting}
...
\end{lstlisting}
\saveSpace
\noindent If the former code were accepted, 
