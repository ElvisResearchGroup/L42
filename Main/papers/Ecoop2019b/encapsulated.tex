...
As you can se\IO{e, i}t would be possible for external code with no knowledge of the \Q@ShippingList@ to mutate its items.%
\footnote{%
...
\IO{Removing} our restrictions on capsule mutators \IODel{ensure that capsule fields are essentially an exclusive mutable reference.
...
\end{lstlisting}\IOComm{Merge this and the previous example?}
...
mut method imm Void addHeavy(read Foo f) {
  ...
  f.hi() } // Can 'this' be observed here?
...
mut ShippingList l = new ShippingList(%\IO{new Items()}%);
