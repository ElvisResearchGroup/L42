...
\IO{We have found} our design, \REV{using an inner \Q@Box@ object, \IO{to be quite a useful pattern in static verification}\IODel{is a common pattern in static verification}}{C}{I thought I knew a bit about static verification, but I've never heard of the Box pattern or seen anything like it.}: where one encapsulates all relevant mutating state into an encapsulated subobject which is not exposed to users.
