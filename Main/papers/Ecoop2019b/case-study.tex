...
%  validate
%}
Here we show that we are able to verify classes with circular mutable object graph\IO{s, t}hat interact with the real world using I/O.
...
\end{lstlisting}\IOComm{Later on we talk about the width and height \emph{fields}, however the above code dosn't have them!}
...
In order to perform this case study we had to first implement a simple GUI Library in L42. This library uses object capabilities to draw the widgets on screen, as well as fetch and dispatch the events. Importantly, neither our application, nor the underlying GUI library require\IO{s} back door\IO{s, i}nto either our type modifier or capability system to function, demonstrating the practical usability of our restrictions.
...
public (in D) and qualified\footnote{That is, the receiver is not \Q!this!.} (in Eiffel) method call\IODel{s}.
In our \Q!SafeMovable! class, all methods are public, and all calls \IO{(outside the invariant)} are qualified, thus this difference is irrelevant. Neither protocol performs invariant checks on field accesses or updates,
...
%}
, Eiffel allows fields to directly implement methods, allowing the \Q!width! and \Q!height! \emph{fields} to directly implement \Q!Widget!'s \Q!width! and \Q!height! \emph{methods}. On the other hand in D, one would have to write getter \emph{methods}, which would \IODel{invoke} \IO{perform} invariant checks.
...
\IO{As} the backend of the L42 GUI library is written in Java, we did not port it to Spec\#, rather we just simulate\IO{d it} \IODel{the backend}, and don't actually display a GUI in Spec\#.
