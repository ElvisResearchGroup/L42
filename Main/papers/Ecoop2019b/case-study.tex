...
%-not overlap with anyhing else inside
%Our containers represent boxes that completley contain other non-overlapping boxes.
This set up is not overly \IO{complicated}, the maximum nesting level of \Q@Widget@s is $5$.
...
As an experiment\IO{,} we set our implementation to generate invariant checks following the  visible state semantics \IO{approaches of D and Eiffel~\cite{Alexandrescu:2010:DPL:1875434,DRef}},
where the invariant \IO{of the receiver} is instead checked at the start and end of \emph{every} 
\IO{public (in D) and qualified\footnote{\IO{That is, the receiver is not \Q!this!.}}} (in Eiffel) method calls.
\IO{In our \Q!SafeMovable! class, all methods are public, and all calls are qualified, thus} this difference is irrelevant. \IO{Neither protocol performs invariant checks on field accesses or updates,
however due to the `uniform access principle'%\footnote{%
%L42 \IO{also} follows the Eiffel uniform access principle: field accesses are the same as method calls.%
%}
...
\IODel{(i.e. on \Q@this@) of \Q@SafeMovable@, including the simple getters implementing the \Q@Widget@ interface}. When we ran our test case following \IODel{this} \IO{the D} approach, the \IO{\Q!invariant! method} \IODel{invariant} was called $52,734,053$ times, whereas the Eiffel approach `only' called it $14,816,207$ times;%\footnote{%
...
This aligns the code with L42, which also performs such checks at runtime.}: that the invariant of \Q@SafeMovable@ was not known to hold at the end of its constructor and \Q@dispatch@ metho\IO{d. L}ike our system howeve\IO{r, S}pec\# checks the invariant
at those two points at runtime. \IODel{As such} \IO{Thus} the code is equivalently verified in both Spec\# and L42; in particular it performed exactly the same number ($77$) of runtime invariant checks.\footnote{%
