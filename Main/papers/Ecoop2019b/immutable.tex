...
\IO{Our protocol enforces that all classes have an }
\IODel{its }\Q@invariant@ method \REV{must be of the form \Q@read method Bool invariant() \{..\}@}{2}{Can  a program write say mut method invariant or is it syntactically [???]}
\IO{that only uses} \Q@this@ to read \Q@imm@ and \Q@capsule@ fields. \IO{If an \Q@invariant@ method is not defined by the programmer, one that always returns \Q@true@ will be generated for them.}
...
\REV{\textit{Unrestricted use of \IO{non-determinism}\IODel{object-capabilities}:}}{3}{Where do OCs factor in to this example? Cite/explain how this example works in Rust and Java.}
\IO{Allowing \Q@invariant@ to (indirectly) perform a non-deterministic operation, such as by creating new capability objects, could allow \Q@invariant@ to be non-deterministic.}%
\IODel{Allowing \Q@invariant@ to (indirectly) use an object capability could make it non-deterministic.} For example consider this simple and contrived (mis)use of person:
...
In particular, note how in the presence of dynamic class loading, we have no way of knowing what the type of \Q@name@ could be. \IO{Since our-system allows non-determinism only through capability-objects, and it prevents the creation of new ones out of thin air, the above example would be prevented.}
...
\REV{If instead we were to not enforce strong exception safety, an invalid object could be made reachable:}{3}{Why not write "bob.invariant()" instead of "bob.name.isEmpty()"?}
\saveSpace
