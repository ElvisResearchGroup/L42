...
In Section~\ref{s:case-study} and Appendix~\ref{s:MoreCaseStudies} we showed how the box pattern can be used to write invariants over cyclic mutable object graphs, the \MS{latter} also shows how a complex mutation can be done in a an `atomic' way, with a single invariant check. However the box pattern is much more powerful. Suppose we want to pass a temporarily `broken' object to other code as well as perform multiple field updates with a single invariant check. 
\MS{One may think we would need to add features
to the language, like } another type-modifier \Q!invalid! (denoting an object whose invariant need not hold), and an \Q!expose! statement like Spec\#:
\begin{lstlisting}
interface Person {
...
Though capsule mutators can be used to perform batch operations like the above, they can only take immutable objects. \MS{This} means that they can perform no non-deterministic I/O (due to our OC system), and other externally accessible objects (such as a \Q!mut Transaction!) cannot be mutated during such an operation.
