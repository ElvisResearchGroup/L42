...
\textit{Example:} we now show a simple example, where we have a \Q@Cage@ class with a list of \Q@Point@s \Q@path@ and \Q@Hamster@ with a \Q@Point@ \Q@pos@; the \Q@Cage@ moves the \Q@Hamster@ across its path. \Q@Cage@ has an invariant: the \Q@Hamster@'s location is within the \Q@Cage@'s path.
...
\textit{Spec\# example:} Here we show the previous example in Spec\# the system most similar to ours (see appendix \ref{s:hamster} for a more detailed discussion about this solution):
