...
We use the guarantees provided by these systems to ensure that:
\begin{itemize}
	\item Invariants are deterministic.
	\item An object's invariant can only refer to objects whose state cannot be mutated by other objects.
	\item Whilst the state an object's invariant refers to is being mutated, that object cannot be used in execution.
	\item An object cannot be used until it is fully constructed.
	\item An object's invariant is checked at runtime, after construction, and after the state it refers to could have been modified; this happens before the object could be used (again).
\end{itemize}
Together, these guarantee that at all times if an object is usable in execution, its invariant holds. See Section \ref{s:protocol} for the exact details as to how we make this guarantee.
