...
%\newpage
%\LINE
\IOComm{Make design goals more explicit!}
...
However this flexibility hampers code reasoning, since \IODel{object behaviour is usually nearly completely
unrestricted. This is further complicated by the support} OO languages typically \IO{support nearly unrestricted use of} \IODel{have for} exceptions,
memory mutation, \IO{object aliasing,} and I/O.
...
We do this, like Dafny~\cite{DBLP:conf/sigada/Leino12}, to minimise special syntactic and type-system treatment of invariants, making them easier to understand for users\IODel{. W}\IO{, w}hereas most other approaches treat invariants as a special annotation with its own syntax.
...
For example, three languages: L42~\cite{ServettoZucca15,ServettoEtAl13a,JOT:issue_2011_01/article1,GianniniEtAl16}, Pony~\cite{clebsch2015deny,clebsch2017orca}, and the language of Gordon \etal~\cite{GordonEtAl12} use Type Modifiers (TMs)\IO{\footnote{\REVm{TMs are called \emph{reference capabilities} in other works. We use the term TM here
		to not confuse them with object capabilities, another technique we also use in this paper.}}}\REVt{D}{I'm not usually one to quibble over terminology, but this use of "Type Modifiers" terminology is
...
However, our approach works in a pure OO setting without such a distinction. Hence we write all type names in \Q@BoldTitleCase@ to underline this. Note: to save space, here and in the rest of the paper we omit the bodies of constructors that simply initialise fields with the values of \IO{the} constructor\IO{'s} parameters, but we show their signature in order to show any annotations.

We use the \Q@read@ annotation on \IO{the} \Q@equals@ \IO{method} to express that it \IODel{does not modify} \IO{modifies n}either \IODel{the} \IO{its}
receiver \IO{nor its} \IODel{or the} parameter. In \Q@Cage@ we use 
the \Q@capsule@ annotation to ensure
that the \IO{modification of the} \Q@Hamster@'s \emph{reachable object graph} (ROG) is fully under the control
...
This technique loosely corresponds to an implicit pack and unpack: we \IODel{use} \IO{`unpack'} \Q@this@ only to read the field value, then we work on its value while the invariant of \Q@this@ is not known to hold, finally \IO{when returning, we `pack' \Q!this! and} \IODel{we} check \IODel{the} \IO{its} invariant before allowing \IODel{the object} \IO{it} to be used again.
...
In both versions, we designed \Q@Point@ and \Q@Hamster@ in a general way, and not solely to be used by classes with an invariant, \IODel{in particular} \IO{as such, } \Q@Point@ is not an immutable class. However, doing this in Spec\# proved difficult, in particular we were unable to override \Q@Object.Equals@, or even define a usable \Q@equals@ method that takes a \Q@Point@, as such we could not call either \Q@List<Point>.Contains@ or \Q@List<Point>.IndexOf@.
...
\subheading{Summary}
We have fully implemented our protocol in L42\footnote{A suitably anonymised, experimental version of L42, supporting the protocol described in this paper, together with the full code of our case studies, is available at \url{http://l42.is/EcoopArtifact.zip} \IOComm{Change filename to \Q!InvariantArtifact.zip! (so it makes sense for the OOPSLA submision as well)}. We also believe it would be easy to implement our protocol in Pony and Gordon \etal's language.}\IODel{\footnotemark}, we used this implementation to implement and test an interactive GUI involving a class with an invariant. On a test case with $5$ objects with an invariant, 
...
Section \ref{s:TMsAndOCs} explains the \IO{pre-existing} \emph{type modifier} and \emph{object capability} \IO{features} \IODel{support} we use for this work.
...
case studies and comparisons against Spec\#, D and Eiffel. \IOComm{Summarise the results of these extra comparisons!}
