...
Appendix \ref{s:proof} provides a proof that our invariant protocol is sound. \IODel{\NL} Appendix~\ref{s:hamster} explains exactly why the Spec\# \Q!Hamster! encoding was so verbose. In Appendix~\ref{s:MoreCaseStudies} we designed a worst case scenario were Spec\# 
...
In order to compare with examples from other \IO{work} \IODel{authors}, in Appendix~\ref{s:MoreCaseStudies} we exam\IO{in} the examples of some Spec\# papers~\cite{DBLP:journals/jot/BarnettDFLS04,leino2004object,DBLP:conf/mpc/BarnettN04}; we show \IO{why we can not encode some of them} \IODel{that we can not encode some of those examples}.
At first glance, our approach may feel very restrictive\IODel{. I}\IO{; }in Appendix~\ref{s:patterns}, we show
%The name Box pattern is not introduced yet
programming patterns  \IO{demonstrating that these restrictions do not significantly hamper expressivity} \IODel{supporting very expressive programming patterns}. \IO{In Appendix \ref{s:runtime-verification}, we discuss related work on runtime verification.}
...
\IOComm{For OOPSLA mention extended related work appendix.}
