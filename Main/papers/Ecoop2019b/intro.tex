...
\IO{Appendix~\ref{s:hamster} explains exactly why the Spec\# \Q!Hamster! encoding was so verbose. In Appendix~\ref{s:MoreCaseStudies} \MS{we designed a worst case scenario} were Spec\# 
...
\MS{In order to compare with examples from other authors, in Appendix~\ref{s:MoreCaseStudies} we exam the examples of some Spec\# papers~\cite{DBLP:journals/jot/BarnettDFLS04,leino2004object,DBLP:conf/mpc/BarnettN04}; we show that we can not encode some of those examples.}
\MS{At first glance, our approach may feel very restrictive.} In Appendix~\ref{s:patterns}, we show} 
%The name Box pattern is not introduced yet
\MS{programming patterns supporting very expressive programming patterns.}
