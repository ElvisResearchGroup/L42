...
Appendix \ref{s:proof} provides a proof that our invariant protocol is sound.

\IODel{Appendices \ref{s:hamster} and \ref{s:MoreCaseStudies} provide further
case studies and comparisons against Spec\#, D and Eiffel.}
\IO{Appendix~\ref{s:hamster} explains exactly why the Spec\# \Q!Hamster! encoding was so verbose. Appendix~\ref{s:MoreCaseStudies} shows an additional case study were Spec\# only performs half as many invariant checks as our protocol, whereas D and Eiffel only perform twice as many; we additionally discuss how Spec\# is able to represent more kinds of invariants than our protocol. Finally, in Appendix~\ref{s:patterns}, we show how the box pattern can be used to perform batch operations with a single invariant check, as well as how the state of `broken' objects can be passed around.} \IOComm{For OOPSLA mention extended related work appendix.}
