...
If \Q@pos@ is not known to be consistent, one will be unable to mutate it, by updating one of its fields or by passing it as an argument (or receiver) to a \IO{non-}\Q@Pure@ method.
Though we don't want \Q@pos@ to ever mutate, Spec\# currently has no way of enforcing that an \emph{instance} of a \IO{non-}immutable class is itself immutable\footnote{There is a \IO{paper~\cite{DBLP:conf/vstte/LeinoMW08} that} \IODel{the} describes a simple solution to this problem: assign ownership of the object to a special predefined `freezer' object, which never gives up mutation permission\IODel{~\cite{DBLP:conf/vstte/LeinoMW08}}, however this does not appear to have been implemented; this would provide similar flexibility to the TM system we use, which allows an initially mutable object to be promoted to immutable.}, as such we will simply refrain from mutating it.
...
\IO{If however, we did} \IODel{We don't} want \Q@Point@ to be an immutable/value type\IODel{, however if it were}, the original unannotated version would not have any problems.
