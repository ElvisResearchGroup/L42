...
% TODO: Add ctor declarations
% TODO: Make Spec# use types in System, not keywords
\begin{lstlisting}
class Person { 
  final String name;
  Int age;
...
class Family { 
  static class FBox { 
    ...
    mut method Void processDay(Int day) {
      ...
      for (Person p : this.children) { p.processDay(day); }
    }
  }
  capsule FBox box;
  ...
  Family(capsule List<Person> parents, 
    ...
    this.box = new FBox(parents, children); }
...
for (Int day = 0; r < 3*365; day++) { // Run for 3 years
  fam.processDay(day);
  if (r == 365 + 45) {
    fam.addChild(new Person("Tim", 0, day % 365)); }
  if (r == 2*365 + 340) {
    fam.addChild(new Person("Diana", 0, day % 365)); }}
\end{lstlisting}
