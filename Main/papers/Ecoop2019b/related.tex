...
\IOComm{Background on capsule fields, relation with owner as modifier?}
...
each of these works is accompanied by proofs about the properties of those modifiers. \IOComm{The Pony ones have no proofs!}
...
TM approaches like Javari~\cite{TschantzErnst05,Boyland06} and \REV{Rust}{A}{Why? Can you expand what the problem is?}~\cite{matsakis2014rust} are unsuitable since they introduce back doors which are not easily verifiable as being used properly. \IOComm{This is problematic, because TMs are very general! In particular, the way Rust wants them to be used is unsuitable, even without back doors. If we deleted interior mutability or made their exclusive/mutable references deep, however it might work.}
...
\IOBlock{Why does this paragraph exist? It's unrelated to anything.}{Capsule/isolated local variables are affine in that they can be used only once, however this linearity is a property of variables, not expressions or fields. Linear/affine types extend this idea further, however they usually do not consider the ROGs of such types, or work in an OO setting~\cite{ahmed20073,fahndrich2002adoption}.}
...
\item Or, as in this work, the invariant of any object which could be \emph{involved} in execution is assumed to hold. It is checked after every modification of the object or its encapsulated ROG. \IOComm{Is a reading going to know what an 'encapsulated ROG' is?}
\end{itemize}
...
and JML~\cite{Burdy2005}). \IOComm{This sentence is unconnected to the rest of the paragraph, it also dosn't say anything interesting.}
...
Chaperones and impersonators~\cite{DBLP:conf/oopsla/StricklandTFF12} lift\IODel{s} the techniques of gradual typing~\cite{takikawa2015towards,DBLP:conf/oopsla/TakikawaSDTF12,DBLP:conf/popl/WrigstadNLOV10}
...
This is in contrast \IODel{with} \IO{to} the main strategy of static verification: to verify a method, the system assumes the contracts of other methods, and the content of those contracts is the starting point for their proof.
...
soundly handled by our protocol, independently of the \IO{behaviour of} code that uses such class or any other surrounding code.
...
Dafny is also generally highly restrictive with its rules for mutation\IODel{,} and object construction, it also does not provide any means of performing non deterministic I/O.
...
As you can se\IO{e, f}ailing to handle non-determinism jeopardises reasoning.
