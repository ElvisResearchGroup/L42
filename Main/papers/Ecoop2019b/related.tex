...
This is like a mathematical proof: a program is valid if it is all correct, but a single error invalidates all claims. This makes it hard to perform verification on large programs, or when independently maintained third party libraries are involved.
This does not happen with a type system, since its properties are more coarse grained, simpler and easier to check.
 Static verification has more flexible and fine grained annotations and is more fragile since it uses a theorem prover as a backend.}{3}{This hole discussion seems muddled.}
...
You can see our approach as an extremely modularized	version of such system: every class is its own \REV{demilitarized}{3}{?} zone, and the rest of the code could have Byzantine behaviour. Our formal proofs show that every class that compiles/type checks is 
