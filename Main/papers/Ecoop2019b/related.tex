...
\REV{Many works attempt to move out of the `RV tool' philosophy to ensure RV monitors work as expected, as for example
%\sepItems
...
the study of contracts as refinements of types~\cite{findler2001contract}.}{3}{Dose not offer much explanation or comparison}.
\MS{However, such work is only interested in pre and post conditions, not class invariants.}
...
%\LINE
\REV{\emph{Chaperones and impersonators}~\cite{DBLP:conf/oopsla/StricklandTFF12} lift the techniques of gradual typing~\cite{takikawa2015towards,DBLP:conf/oopsla/TakikawaSDTF12,DBLP:conf/popl/WrigstadNLOV10}
...
Their approach allows wrapping invariant operations around method calls, as is done in aspect oriented systems like Jose~\cite{feldman2006jose}.}{3}{Dose not offer much explanation or comparison}
\MSComm{I think this comparison is sufficient}
...
%\LINE
\textit{\MS{Security and scalability}:}
Our approach allows verifying an object's invariant independently of the actual invariants of other objects.
This is in contrast with the main \MS{strategy} of static verification: to verify a method, the system assumes the contracts of other methods, and the content of those contracts is the starting point for their proof.
\MS{Thus, static verification proceeds like} a mathematical proof: a program is valid if it is all correct, but a single error invalidates all claims. This makes it hard to perform verification on large programs, or when independently maintained third party libraries are involved.
This does not happen with a type system, since its properties are more coarse grained, simpler and easier to check.
 Static verification has more flexible and fine grained annotations and \MS{often relies} on a fragile theorem prover as a backend.
\MSComm{above: fixing REV3:This hole discussion seems muddled.}
...
\REV{To soundly verify code embedded in an untrusted environment, as in gradual typing~\cite{DBLP:conf/oopsla/TakikawaSDTF12,DBLP:conf/popl/WrigstadNLOV10}, it is possible to 
...
\MS{own verified core}, and the rest of the code could have Byzantine behaviour. Our formal proofs show that every class that compiles/type checks is 
soundly handled by our protocol, independently of the code that uses such class or any other surrounding code.}{2}{Why no static core then?}
...
\REV{\textit{Static Verification:}}{3}{ It would be better to discuss this tradeoff between expressiveness/usability much earlier, and for it to be addressed through a larger, and possibly more balanced, evaluation with Spec\#.}
