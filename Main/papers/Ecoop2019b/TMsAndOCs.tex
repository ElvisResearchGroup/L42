...
How \Q!capsule! fields are handled differs widely in the literature. In order for \Q!capsule! fields to always contain a unique reference to their ROG, the language of Gordon \etal only allows them to be read destructively (i.e. by replacing the fields old value with a new one, such as \Q!null!). 
...
\noindent This strategy does not restrict when exceptions can be \emph{thrown}, but only \IO{restricts when unchecked exceptions can be \emph{caught}} \IODel{catching unchecked ones}.
