...
made particularly difficult by mutation, aliasing, dynamic dispatch, I/O, and exceptions. There are many ways to perform such reasoning, here we use the type system to \IODel{control} \IO{restrict}, but not prevent\IODel{,} such behaviour in order to be able to soundly enforce invariants with runtime verification (RV).
% \IO{[dynamic class loading]},

\subheading{Type \IO{M}odifiers (TMs)}
TMs as used in this paper are a type system feature that allows reasoning about aliasing and mutation\IO{. R}ecently a new design for them has emerged that radically improves their usability\IO{;
t}hree different research languages are being independently developed relying on this new design: the language of Gordon et.~al.~\cite{GordonEtAl12}, Pony~\cite{clebsch2015deny,clebsch2017orca}, and L42~\cite{ServettoZucca15,ServettoEtAl13a,JOT:issue_2011_01/article1,GianniniEtAl16}.
These project\IO{s} are quite large: several million lines of code are written in Gordon et.~al.'s language and are used by a large private Microsoft project; Pony and L42 have large libraries and are active open source projects. In particular the TMs of these languages are used to provide automatic \IO{and} correct parallelism~\cite{GordonEtAl12,clebsch2015deny,clebsch2017orca,ServettoEtAl13a}.
...
\begin{itemize}
\item Mutable (\Q@mut@): the referenced object can be mutated, as in most imperative languages without \IO{modifiers.}
If all types \IO{are}\IODel{were} \Q@mut@, there \IODel{would be} \IO{is} no restriction on aliasing/mutation.
\item Readonly (\Q@read@): the referenced object cannot be mutated by such references, but there may be mutable aliases to \IODel{that} \IO{such} object, \IODel{so} \IO{thus} mutation can still be observed. 
\item Immutable (\Q@imm@): the referenced object can never mutate. Like \Q@read@ references, one cannot mutate through an \Q@imm@ reference\IO{, however} \Q@imm@ references also guarantee that the referenced object will not mutate through any \IODel{reference} \IO{other alias}.
\item \REV{\IO{Encapsulated} (\Q@capsule@):
 everything in the ROG of a capsule reference (including itself) is mutable only through that reference; however immutable references can be freely shared across capsule boundaries.}{2}{how easy is it for the programmer to use capsule?} \IOComm{what about read?}
\end{itemize}
...
%\loseSpace
TMs are different to field or variable modifiers like Java's \Q@final@: TMs apply to references, whereas \Q@final@ applies to fields themselves. Unlike \IO{a variable/field of an} \Q@imm@ \IO{type}, a \Q@final@ variable/field cannot be reassigned, it always refers to the same object\IO{, however} the referenced object itself may be mutated.
On the other hand, an \Q@imm@ reference refers to an object that will never be mutated\IO{, but} an \Q@imm@ variable can be reassigned.\footnote{In C, this is similar to the difference between \Q@const *A@ and \Q@*const A@, where a \Q@final read@ variable would be like a \Q@const *const A@.}
%\end{itemize}
...
%  \item no \emph{down}-casting is allowed between different type modifiers.
  \item no casting\IO{/promotion} from \Q@read@ to \Q@mut@ is allowed.
...
and \Q@mut@ nearly everywhere else\IO{. T}he system will be able to transparently promote/recover~\cite{GordonEtAl12,clebsch2015deny,ServettoZucca15} the type modifiers, adapting them to their use context.
...
\end{lstlisting}
\saveSpace
Here \Q@mc@, \Q@ic@, and \Q@cc@ are syntactically initialised with the same expression\IODel{: \Q@new Circle(..)@}.
...
allows \IODel{promoting} the \Q@mut@ result to \IO{be promoted to} \Q@capsule@\IO{, t}hus \Q@cc@ is ok. 
...
Former work~\cite{Boyland10,boyland2003checking,Hogg91,Smith:2000:AT:645394.651903,DBLP:conf/pldi/AikenFKT03}\IO{, which} \IODel{that} eventually enabled the work of Gordon et.~al., does not consider promotion and 
...
these TMs are opt\IODel{-}in: a programmer can write large sections of code
...
Capsule references are usually expressed using linear/affine types~\cite{boyland2001alias} when they are passed around and manipulated. Both \IO{the} capsule/isolated fields and variables \IO{of} Pony and Gordon et.~al.\IODel{'s} rely on destructive reads~\cite{GordonEtAl12,clebsch2015deny}, i.e reading such fields replaces their values with \Q@null@\IO{, a}nd capsule local variables can only be used once. \IO{Capsule local \emph{variables} are also linear in L42, however} \Q!capsule! \IO{\emph{fields}}~\cite{ServettoEtAl13a,ServettoZucca15} do not require destructive reads, but provide sightly different guarantees \IO{compared} \IODel{with respect} to capsule local variables, where the ROG of a \Q@capsule@ field can be referenced from outsid\IO{e,} but only in controlled ways.
...
if a \Q@try@\IO{--}\Q@catch@ block caught an exception, the state visible before execution of the \Q@try@ block is unchanged, and the exception object does not expose any object that was being mutated.
%\LINE
...
\item Only \IODel{an} \Q@imm@ object\IO{s} may be thrown as \IODel{an} unchecked exception\IO{s}.
\end{itemize} 
...
the broken object is guaranteed to be garbage collectable when the exception is captured. For the purposes of soundly catching invariant failure\IO{s,} it would be sufficient to enforce SES only when capturing exceptions caused by \IODel{\Q@invariant@} \IO{such} failures.
...
\subheading{Object \IO{C}apabilities (OCs)}
OCs, which L42, Pony, and Gordon et.~al.'s work \IODel{use} \IO{have}, are a widely used~\cite{miller2003capability,
noble2016abstract,karger1988improving} programming style that allows associating resources with objects. When this style
is respected, code that does not possess an alias to \IODel{the} \IO{such an} object cannot use \IODel{the} \IO{its associated} resource.
...
Here, as in Gordon et.~al.'s work, we use OCs to reason about determinism and I/O. To properly enforce this, the OC style needs to be respected while implementing the primitives of the standard library and \IODel{native} \IO{when performing foreign function} calls that could be non deterministic, such as operations that read from files or generate random numbers. Such operations would not be provided by static methods, but instead instance methods of classes whose instantiation is kept under control. 
...
OCs are typically not part of the type system nor do they require runtime checks or special support beyond that provided by a memory safe language\IO{. However,} since
L42 allow user code to perform \IODel{native} \IO{foreign} calls without going through a predefined standard library, its type system enforces the OC pattern over such calls:
%To reason about determinism, L42 connects TMs with the OC style as follows: % style by requiring:
\begin{itemize}
\item \IO{Foreign} \IODel{Native} methods (which have not been white listed as deterministic) and methods whose name start with \texttt{\#\$} are \emph{capability methods}.\IODel{${}^{11}$}%
\item Constructors of classes declared as \emph{capability classes} are also capability methods.
\item Capability methods can only be called by other capability-methods or \Q@mut@/\Q@capsule@ methods of capability classe\IO{s.}
\item In L42 there is no \Q@main@ method, \IODel{but} \IO{rather it has several} main expressions; such expressions can also call capability methods, thus they can instantiate capability objects and pass them around \IO{to} the rest of the program.
...
For the purposes of invariant checking, we only care about the effects that methods could have on the running program and heap. As such, \emph{output} \IODel{native} methods (such as a \Q@print@ method) can be white listed as `deterministic', provided they do\IODel{es} not effect program execution, such as by non deterministically throwing I/O \IODel{exceptions} \IO{errors}.
...
as being deterministic and not mutating existing memory.
\begin{itemize}
	...
	\item no other preexisting objects are accessible (as L42 does not have global variables).\footnote{%
		\IO{If} L42 \IODel{does not} \IO{did} have static variables, \IODel{if it did} getters and setters for them would be capability methods.
