...
every non-immutable object in the reachable object graph (ROG) of a capsule reference (including itself) is only reachable through that reference. This means that if a capsule reference $r$ is usable in the same expression as a reference $r'$, then either $r'$ does not refer to an object reachable from $r$, or $r'$ refers to an immutable object. Note an encapsulated reference can be freely used as either mutable or immutable, since there could have been no other references to it.
 In order to ensure \Q!capsule! variables uniquely refer to an object, their access must be restricted. In Pony and Gordon \etal's work, reading a \Q!capsule! variable does not produce a \Q!capsule! reference, however they provide destructive reads that return the variable's old value as \Q!capsule!. L42 however uses a simpler approach, where all accesses to \Q!capsule! local variables consume them: they are expressed using linear/affine types~\cite{boyland2001alias}, thus they can only be used once. \IO{Later on, we discuss capsule fields, which behave slightly differently.}
%\MS{This discussion apply to capsule local variables and capsule references. We will discuss details on capsule fields below.}
\end{itemize}
...
\IO{How \Q!capsule! fields are handled differs widely in the literature. In order for \Q!capsule! fields to always contain a unique reference to their ROG, the language of Gordon \etal only allows them to be read destructively (i.e. by replacing the fields old value with a new one, such as \Q!null!). 
...
In contrast, Pony does not guarantee that capsule fields contain a unique reference. Non-destructive reads of \Q!capsule! fields produces a non unique-reference the can only be used in controlled ways.
...
This is still useful for safe parallelism as destructive reads of a \Q!capsule! field return a \Q!capsule! reference (which can be sent to other actors), however the ROG of an \Q!capsule! field can be mutated by the same actor, even within methods of unrelated objects.
L42 supports a variation of capsule fields similar to Pony's, but does not support destructive reads~\cite{ServettoEtAl13a,?}. In Section \ref{s:protocol} we present a novel kind of \Q!capsule! field, that instead of preventing data-races, prevents representation exposure.}
...
\IO{SES allows us to throw invariant failures as unchecked exceptions: if an objects ROG was mutated into a broken state within a try block, when the invariant failure is caught, the mutated object will be unreachable/garbage-collectable. This works since SES guarantees that not object mutated within a try block is visible when it catches an unchecked exception.}
