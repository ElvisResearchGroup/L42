...
These project are \REV{\IO{quite} large}{3}{Seems very large to me!}: several million lines of code are written in Gordon et. al.'s language and are used by a large private Microsoft project; Pony and L42 have large libraries and are active open source projects. In particular the TMs of these languages are used to provide automatic/correct parallelism~\cite{GordonEtAl12,clebsch2015deny,clebsch2017orca,ServettoEtAl13a}.
...
\IODel{; \REV{we cannot use a \Q@read@ reference to cause mutation, but we have no guarantee of the absence of mutation:}{2}{repetitive} in our example}\IO{, where} we can observe a change in \Q@rp@ caused by a mutation inside \Q@mp@.
...
\end{lstlisting}
\saveSpace
\REV{\IODel{Here \Q@mc@, \Q@ic@, and \Q@cc@ are syntactically initialised with the same expression: \Q@new Circle(..)@.}}{2}{obvious from the code}
...
the \IO{broken} object is guaranteed to be garbage collectable when the exception is captured. For the purposes of soundly catching invariant failures it would be sufficient to enforce SES only when capturing exceptions caused by \Q@invariant@ failures.
