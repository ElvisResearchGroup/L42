...
\item Encapsulated (\Q@capsule@):
 everything in the reachable object graph (ROG) of a capsule reference (including itself) is mutable only through that reference\IODel{; \REV{however immutable references can be freely shared across capsule boundaries}{B}{what does [this] mean}}. \IO{This means that for any expressions $e$ and $e'$, if $e$ is typeded as \Q!capsule!, and $e'$ is typed as \Q!mut! or \Q!capsule!, then the result of $e'$ will not be in the ROG of $e$; however, if $e'$ is typed as \Q@read@ or \Q@imm@, it may be in the ROG of $e$.}
\end{itemize}
