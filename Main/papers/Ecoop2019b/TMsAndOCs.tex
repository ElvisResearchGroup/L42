...
There are several possible interpretations of the semantics of type modifiers\IO{ as applied to field}.
Here we assume the full/deep meaning~\cite{ZibinEtAl10,Potanin2013}:
\begin{itemize}
  \item \IO{Any field accessed from an \Q!imm! reference produces an \Q!imm! reference; thus all} the objects in the ROG of an immutable object are \IO{also} immutable,
  \item \IO{A \Q!mut!} \IODel{a mutable} field accessed from a \Q@read@ reference produces a \Q@read@ reference\IO{; thus a \Q!read! reference cannot be used to mutate the ROG of the referenced object.}\IODel{,}
%  \item no \emph{down}-casting is allowed between different type modifiers.
  \item \IODel{n}\IO{N}o casting \IODel{/} \IO{or} promotion from \Q@read@ to \Q@mut@ is allowed.
%  \item promotion, is a type-system feature allowing implicit and safe casting from \Q@read@ and \Q@mut@ to \Q@imm@.
\end{itemize}

\IO{How \Q!capsule! fields are handled however differs widely in the literature. In order for \Q!capsule! fields to always contain a unique reference to their ROG, the language of Gordon \etal only allows them to be read destructively (i.e. by replacing the fields old value with a new one, such as \Q!null!). In contrast, the language of Pony does not provide a true \Q!capsule! field, rather it allows \Q!iso! fields which grantee that no \emph{variable} or \emph{field value} outside the ROG of such a fields value, has read or write access to a non-immutable object reachable from the \Q!iso! field. This is still useful for safe parallelism as destructive reads of an \Q!iso! field return a proper \Q!capsule! (which can be sent to other actors), however the ROG of an \Q!iso! field can be mutated by method calls within the same actor, but on unrelated objects.
L42 supports a variation of capsule fields similar to Pony's, but does not support destructive reads~\cite{ServettoEtAl13a,?}. For invariant checking, we would like to restrict which \emph{objects} (as opposed to \emph{threads}) can mutate the ROG of a \Q!capsule! field, in Section \ref{s:protocol} we present a novel design of \Q!capsule! fields that does this.}

\noindent\IO{\subheading{Promotion and Recovery}}
...
\IODel{Here \Q@mc@, \Q@cc@, and \Q@ic@ are syntactically initialised with the same expression: \Q@new Circle(..)@.
...
any expression of a \Q@mut@ type that uses no free \Q@mut@ variables declared outside can be implicitly promoted to \Q@capsule@/\Q@imm@.\footnote{\IODel{
This requires some restrictions on \Q@read@ fields not discussed in detail for lack of space.
}}}

\IO{In this paper, as in~\cite{clebsch2015deny,?}, \Q!new! expressions always return a \Q!mut!, so the declaration for \Q!mc! is well typed. The flexible TM system we use allows the \Q!mut! result of such an expression to be implicitly promoted to \Q!capsule! or \Q!imm! (as in the above declarations for \Q!cc! and \Q!cc!). Such promotions are allowed for \emph{any} expression that does not have \Q!mut! or \Q!read! free variables\footnote{\IO{This assume that there are no \emph{global} \Q!read! or \Q!mut! variables/constants, as in L42, Pony, and Gordon \etal's language.}}. Since such a expression uses no \Q!mut! variables, it cannot store references to its results ROG on the heap, similarly if it used a \Q!read! (or \Q!mut!) variable, it cannot capture an existing mutable object in its ROG.}
\IOComm{Note that Gordon and Pony freely allow read fields, so the promotion would not work if the new expressions used \Q!read! variables. L42 also allows this to work with read variables, but I don't think it's important enough to mention.}
...
\IODel{The \Q@capsule@ modifier (sometimes called \Q!isolated! \IODel{/\Q@iso@} \IO{or} \IO{\Q!baloon!}) is possibly the one whose details differ the most in the literature.} \IODel{\IOBlock{Moved!}{Here we refer to the interpretation of \ldots. This concept is the basis for L42, Pony, and Gordon \etal's type systems \ldots.}}
%\begin{itemize}
...
\IODel{The capsule/isolated fields of Gordon \etal and Pony rely on destructive reads~\cite{GordonEtAl12,clebsch2015deny}: in order to read them, a new value (such as \Q!null!) will be assigned to them. In contrast, L42~\cite{ServettoEtAl13a,ServettoZucca15} does not require such destructive reads, thus \Q@capsule@ fields can be accessed many times, and their content can be seen from outside; but only in controlled ways. Both Gordon \etal and Pony restrict how \Q!capsule! local variables can be used by changing the type they are seen as, however both allow the local variable to be `consumed', allowing them to be used as normal capsule/isolated expressions, at the cost of being unable to use the variable again. \IOBlock{Moved!}{L42 however uses a simpler approach \ldots.}}
...
\begin{itemize}
\item Code inside a \Q@try@ block \IODel{capturing} \IO{that captures} unchecked exceptions is typed as if all \Q@mut@ variables declared outside of the block are \Q@read@.
...
\noindent This strategy does not restrict \IO{when} \IODel{throwing} exceptions \IO{can be thrown}, but only catching unchecked ones.
\IO{SES allows us to throw invariant failures as unchecked exceptions: an object that was mutated into a broken state is guaranteed to be unreachable/garbage-collectable when the invariant failure is caught} \IODel{SES allows us to soundly capture invariant failures as unchecked exceptions: the broken object is guaranteed to be garbage collectable when the exception is captured.}	
...
noble2016abstract,karger1988improving} programming \IO{technique where access to resources are encoded as objects} \IODel{style that allows associating resources with objects}. When this style
...
\item In L42 there is no \Q@main@ method, rather it has several main \IODel{expressions} \IO{\emph{expressions}}; such expressions can also call capability methods, thus they can instantiate capability objects and pass them around to the rest of the program.
...
\end{itemize}
\IO{We are unsure about the details of Gorodn \etal's and Pony's OC style, and if they can be used to enforce purity.}
