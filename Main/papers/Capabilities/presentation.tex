...
We will consider C♯ (or any other .Net or JVM language), since it:\pars
\begin{itemize}
...
\begin{enumerate}
\item What \emph{could} this function do?
...
$\texttt{\ensuremath{\tab\cv{\cv{code}.\cf{GetMethod(}\cl{"M"}\cf )}}.\ensuremath{\cf{Invoke(}\ck{null}}, \ensuremath{\ck{null}\cf )}; \} \ensuremath{\cm{// dynamically}} \ensuremath{\cm{invoke}} \ensuremath{\cf{M()}}}$
...
\pause{}
\item $\cf{Restricted}$ can only be directly called by functions that contain
$\cf{Restricted}$ in their $\ck{calls}$:
...
$\texttt{\ensuremath{\ck{static}} \ensuremath{\ck{void}} \ensuremath{\cf{M2(}\ct{I<\cf{[Restricted]}>}} \ensuremath{\cv x}) \ensuremath{\ck{calls[\cf{Restricted}]}} \{ \ensuremath{\cv x}.\ensuremath{\cf{Run()}}; \}}$

\pause{}
\item Defer the decision to the caller of $\cf{M2}$:

$\texttt{\ensuremath{\ck{static}} \ensuremath{\ck{void}} \ensuremath{\cf{M2<'a>(}\ct{I<\cf{['a]}>}} \ensuremath{\cv x}) \ensuremath{\ck{calls[\cf{'a}]}} \{ \ensuremath{\cv x}.\ensuremath{\cf{Run()}}; \}}$
\end{enumerate}
...
\item Have a $\ck{cant-call}$ annotation?
...
\begin{itemize}
\item Reason in the presence of unsafe operations (like executing arbitrary
machine code).
\item Prove them!
\end{itemize}
\end{itemize}
