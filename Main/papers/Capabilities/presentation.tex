...
\renewcommand{\thefooter}{%
		...
		\begin{frame}%
		\textit{Calls}{\itshape \rmfamily Calls}$\mathit{Calls}$$Calls$
		\maketitle%
		\end{frame}%
	}
}%
...
\setbeamertemplate{block example begin}{%
	...
	\savebox{\mybox}{\usebeamercolor{block title example}\colorbox{bg}{\color{fg}%
			\usebeamerfont{block title example}\rule{0cm}{1em}\insertblocktitle:\hspace{0.5em}\strut}}%
...
\setmainfont[Ligatures={Common, Discretionary}, Scale=1]{TeX Gyre Schola} % 11.5/11
\setmonofont[Ligatures={Discretionary}, Scale=1.0454545454545454]{Consolas} % 11.5/11
\setsansfont[Ligatures={Discretionary}]{Segoe UI} %11/11

\unimathsetup{math-style=ISO}
\setmathfont[Scale=1.045]{TeX Gyre Schola Math}
...
\pause{}
\begin{enumerate}
\item \normalsize  $\f g\in Calls(\f f)\Rightarrow\f f\rightsquigarrow\f g$, i.e. $\f f$
