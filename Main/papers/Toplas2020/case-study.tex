...
not all the code examples come from us. \IOComm{Put one Case Study section and have something like this as an introductory paragraph?}
%We first present
%most of these examples in  Appendix~\ref{s:MoreCaseStudies}, and instead focus here on a single example of a GUI.
We start by presenting our GUI example, demonstrating that we are able to verify invariants over cyclic mutable object graphs in programs that interact with the real world using I/O. \IOComm{Long and convoluted sentence?}
Our example is particularly relevant sine, as with most GUI frameworks, it uses the \emph{composite} programming pattern; arguably one of the most fundamental patterns in OO.
