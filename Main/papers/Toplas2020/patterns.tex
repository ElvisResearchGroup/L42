...
if a capsule mutator on \Q@b@ is active, \Q@a@'s \Q@subInvariant()@ will hold.
...
need to reason over the semantics of the sub invariant pattern, whose complexity is similar to the visible state semantics (as discussed above). \IOComm{I'm not sure I get what this last part is trying to say}

\IOComm{Move the code up/merge into the discussion above?}
\begin{lstlisting}
...
Note how there are only a minimal set of operations defined in the above code, % In English something has "no logic" is like saying it's stupid or dumb (i.e. don't say it unless your intent is to insult)
...
%In this example we show how to encode a \Q@Graph@ 
...
\IOComm{I still think this ought to be in the introduction and/or conclusion}
Note that our goal is not to verify existing code or programming patterns, but to create a simple system that allows soundly verifying the correctness of such data structures.
In particular, as our examples demonstrate, in order to use our approach to ensure invariants, one has to program in an uncommon and very defense style.
...
%Our goal is not to verify existing code or existing programming patterns, but to create a novel programming environment, where with a bit of experience, any data structure can be defined. %\IOComm{Um what? Any languages allow "any data structure to be defined", and simply wanting a "novel programming environment" isn't really a goal}
%Any invariant relying only on immutable and encapsulated state can be enforced; and any transformation based on immutable and capsule input can be defined. \IOComm{put this whole paragraph in the introduction and/or conclusion}
%\IOComm{I'm very confused actually, we should discuss this in person as it seems to be quite different to what you've always tried to say: you want soundness and a "simple" language}
