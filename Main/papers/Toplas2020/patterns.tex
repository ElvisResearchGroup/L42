...
%However, the \Q@subInvariant@ is likely to hold even when there is no reachable reference to \Q@b@ in the current stack frame. % No! We have no reasom to assume this, as programmars could apply the subInvariant pattern in may different ways
...
need to reason over the semantics of the \Q@subInvariant@ pattern, whose complexity is similar to the visible state semantics (as discussed above). \IOComm{I'm not sure I get what this last part is trying to say}
