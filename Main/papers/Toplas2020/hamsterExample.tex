...
The \Q@move@ method actually moves the hamster along the path, but to ensure that our restrictions on \Q@capsule@ fields are respected we forward some of the behaviour to separate methods: \Q@pos()@ which returns the position of \Q@h@ and \Q@moveTo(p)@ which updates the position of \Q@h@.
