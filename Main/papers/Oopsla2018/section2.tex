...
\name classes are roughly equivalent to final classes in Java.  Thus,
compared to Java, the most noticeable difference is the
...
the corresponding call in the Java code typecheck since Java guarantees that such occurrence of \Q@this@ will be a subtype of \Q@A@ everywhere is reused.
...
\textbf{Interfaces are the only way to obtain subtyping}; 
...
We found that the code with \Q@IA@ better transmits the intention of the programmer. Some
