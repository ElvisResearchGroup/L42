...
\begin{grammar}
	\produ{\Sigma}{\State{Dz}{T}}{compilation state}
	\midrule
...
We now recursively define notations to lookup and modify classes from a set of top-level declarations or compilation state:
...
\begin{irules}
	...
	\irule{Redirect}{
		...
		\State{\Dz}{T} \vdash \red{LV}{R} \Rightarrow   
		\reda{R'}{T}{LV\op{remove}{Csz}}
	}
	}\\\\
	\irule{Trait}{
		\cd{\This}{LV}, \Dz; T \vdash \This : \Ok
	}{
		\cd{t}{LV}, \State{\Dz}{T} \vdash t \Rightarrow LV[\This := T]
	}\\\\
	\irule{Sum}{
...
%		p' = (\cd{\Id, LV}, p)[]\\\\
		\summ{LV_1}{LV_2\op{refreshPrivate}{T}} = LV
	}{
		\State{\Dz}{T}\ \vdash \summ{LV_1}{LV_2} \Rightarrow LV
	}
	\end{array}
\end{irules}

\IOComm{The top of the \Rulename{Trait} rule is broken, as thanks to \Rulename{Top}, \Dz could allready have a \This inside it (but I don't care about it, the trait shouldn't be referencing it anyway)}

\IOComm{The \Rulename{Enter} rule is a bit funny, as $L(C)$ could be anything, however we only care that it exists, and what it's private modifier is.}
...
\begin{itemize}
	...
	\item \Rulename{Sum}: this rule simply reduces to the appropriate value. See \autoref{?} for the definition of $\summ{LV_1}{LV_2}=LV$. \IOComm{This rule is stupid, we should move it to the section about sum, and inline the definition}. Where $L\op{refreshPrivate}{T}$ transforms each \ncd{C}{L} in $L$ to \ncd{C'}{L}, and each reference to it (of the form T\.C), to T\.C', for some arbitrary fresh $C'$. This ensures that $\summ{LV_1}{LV_2}$ operates transparently with respect to private nested classes.  
...
\begin{defs}
	...
	\defic{\Sigma \vdash \Sigma(T) : \Ok \text{ and }\Sigma\op{accessible}{T}}{}
	\defis
	\defip{\Sigma\op{accessible}{T\.C}}{}{}%
	\defic{
		\Sigma\op{accessible}{T} \text{ and } \Sigma(T\.C) = \clit{\_}{\_}{\cd{C'}{E}, \_}\text{, or}}{public nested class of an accessible class}
...
%	\defic{\State{\Dz}{T\.Cs}\op{accessible}{T\.Cs'\.C} \text{ and } \Dz(T\.Cs'\.C) = \clit{\_}{\_}{\cd{C'}{E}, \_}}{}
...
A `valid' type is one that can be used as a target in a redirect map. It must be already fully compiled and typed \IOComm{Why do we need it to be typed? Wouldn't compiled (i.e. an $LV$) be sufficient for redirect to make sense?}, and cannot refer to a private class, unless it is an outer class, or directly within one. (See Section \ref{?} for the typing rules of form $\Sigma \vdash $LV$ : \Ok$.)


We now define a way to get the complete set of transitive super-types:

\begin{defs}
	\defi{\Sigma\op{superTypes}{T}}{T,\Sigma\op{superTypes}{Tz}\where \Sigma(T) = \clit{\_}{Tz}{\_}}{}
\defi{\Sigma\op{superTypes}{T_1\ldots T_n}}{\Sigma\op{superTypes}{T_1}\ldots\Sigma\op{superTypes}{T_n}}{}
\end{defs}
\IOComm{Warning! The above \opn{superTypes} definition is very different from the \opn{superClasses} one we use in the algorithm/code, which does an intersection (and not a union) of the superclasses of the $Tz$}
...
\defis
\defip{\Sigma \vdash T \leq \clit{K'}{Tz'}{Mz'}}{}{...}
\defic{\Sigma(T) = \clit{K}{Tz}{Mz}}{...}
...
\begin{defs}
	...
	\defic{\quad \Sigma[Cs] = \clit{\_}{\_}{\cd{C}{L} \_} \imp Cs\.C \in Csz}{}
	\defic{}{(public nested classes must be redirected if their outer is)}
	\defic{\ForAll{Cs\.C\.\_ \in \Csz}{\Sigma[Cs] = \clit{\_}{\_}{\cd{C}{L},\_}}}{(cant redirect through a private class)}
\end{defs}
...
\begin{defs}
	...
	% {K; Tz; Mz, private? C = L}.remove(C.C'.Cs) = {K; Tz; Mz, private? C = L.remove(C'.Cs)}
	% {K; Tz; Mz, private? C = L}.remove(C) = {K; Tz; Mz}
	\defi{\clit{K}{\Tz}{\Mz, \ncd{C}{L}}\op{remove}{C\.Cs}}{\clit{K}{\Tz}{\Mz, \ncd{C}{L\op{remove}{Cs}}}}{}
	\defi{\clit{K}{\Tz}{\Mz, \ncd{C}{L}}\op{remove}{C}}{\clit{K}{\Tz}{\Mz}}{}
\end{defs}
