...
%		p' = (\tld{\Id, LV}, p)[]\\\\
	...
	\irule{Redirect}{
		...
		\forall $T$ \in \ran{R}, p\op{validType}{T}\\\\
...
\begin{defs}
	...
	\defis
	\defip{p\op{accessible}{\This{n}{C}{C'}{Cs}}}{p(\This{n}{C}) = \clit{\_}{\_}{\tld{C'}{E}, \_}}{Not a further nested private}
...
\begin{defs}
\defi{p\op{prepareRedirectMap}{\redm{Cs}{\This{n}{Cs'}}}}{\redm{Cs}{\mini{p}{\This{n+1}{Cs'}}}}{}
...
\begin{defs}
\defip{R' \in p\op{validRedirectMaps}{R}}{}{...}
\defic{R' \in p\op{PossibleRedirectMaps}{R}}{Check that the mapping might work}
\defic{\forall Cs \in \dom{R'} \ p \vdash R'(Cs) : p[R'(Cs)] \leq Cs : R'(p[Cs])}{}
\defic{}{Check that the mapping preserves the structural interface}
\defis
\defip{R' \in p\op{possibleRedirectMaps}{R}}{}{...}
...
\defi{p\op{bestRedirectMap}{R}}{p\op{stableMostSpecific}{R, \opn{validRedirectMaps}}}{...}
\defi{p\op{stableMostSpecific}{R, f}}{R'}{...}
\defic{\wwhere \forall p' \in p\op{similarPrograms}{} \quad p'\op{mostSpecificRedirectMap}{f(p', R)} = R'}{...}
\defic{\text{and } f \text{ is a function from } (p, R) \text{ to } Rz}{...}
\defit{\prog{\DVs}{\DVz,\DVz'} \in (\prog{\DVs}{\DVz})\op{similarPrograms}{}}{}
\defis
\defi{p\op{mostSpecificRedirectMap}{Rz}}{R}{...}
...
\defi{p\op{superClasses}{T}}{T,Tz\where p[T] = \clit{\_}{Tz}{\_}}{}
%MostSpecific(p; Pz) = P
%P in p.minimize(Pz)
...
%  BestRedirectMap(p; R) = StableMostSpecific(p; R; ValidRedirectMaps)
%StableMostSpecific(p; R; f) = R' iff:
%forall p' in SimilarPrograms(p): MostSpecificRedirectMap(p'; f(p', R)) = R'
%with f a function (p,R)->Rz
\end{defs}

\section{Structural Subtyping}
We define a structural subtyping relation $p \vdash T : L \leq Cs : L'$, to mean that $L$ (with name $T$) is a structural `sub-type' of $L'$ (with name $Cs$):

\begin{defs}
\defip{p \vdash T : \clit{K}{Tz}{Mz} \leq Cs : \clit{K'}{Tz'}{Mz'}}{}{...}
\defic{Tz' \subseteq T,Tz \text{, and}}{...}
\defic{\forall m \in \dom{Mz'} \quad p \vdash Mz(m) \leq Mz'(m)\text{, and}}{...}
\defic{\text{if } K = \Q{interface} \text{ then}}{...}
\defic{\qquad K' = \Q{interface} \text{, and}}{...}
...
\IOComm{We should prevent interfaces from having static methods, then we don't need the rule:\\ \indent \indent \Q{if interface?'=empty: if interface? = interface then class not in mwtz'.mdfz}}
