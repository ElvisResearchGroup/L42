...
$c$ of type \Q@Cap@.
This object would have fields modelling, for example, the content of input and output files.
All its methods must require a \Q@mut this@, and the main expression will start with
...
\item The special capability object $c$, is of type \Q@mut Cap@, and it cannot be created with a \Q@new@ expression.
\item We have a special, generated, \emph{monitor} expression \Q@M(@$l$\Q@;@$\e_1$\Q@;@$\e_2$\Q@)@, they are inserted by the reduction as an instrumentation engine that validates.
Initially we will have $\e_2=$ \Q@l.validate()@;
$\e_1$ will be reduced until it becomes a value, then
$\e_2$ will be reduced to test if $l$ is broken. We annotate the monitor-expression with $l$ to proof that if the validation check fails it is precisely $l$ that is invalid. We use a failed monitor expression (i.e. when $\e_2$ is \Q@false@) to represent an \Q@error@  expression.
...
\item We do not have explicit constructor definitions, rather we assume that all constructors are of the form \Q@$C$($T_0$ $x_0$, ...) { this.$f_0$ = $v_0$; $...$}@, where $T_0, ...$ are the types (including modifiers) of the fields of $C$. To provide more flexible initialization one could always make a factory method.
\item We require that \Q@.validate()@ can only use \Q@this@ to access fields, this can be achieved by inlining method calls, or if they are recursive, replace them with calls to methods that take the fields of \Q@this@ instead of \Q@this@ itself.
\item For simplicity, we do not have actual exception objects, rather we just have a concept of an \emph{error} with no associated value. We believe adding traditional exceptions would not cause any interesting variation of our proof.
\end{itemize}
...
For simplicity, we require all field access in method bodies to be of the form
\Q@this.@$f$. Thus we require all fields to be instance-private.
Due to our reduction rules, all field accesses in the main expression will be of the form $l\singleDot\f$.
...
and we not consider subclassing.
...
formed criteria that \Q@validate()@ methods start by calling \Q@super@.\Q@validate()@.
\saveSpace
...
shows that capsule fields section the object graph into a tree of nested ‘baloons’,
...
if $\emptyset;\Gamma,\{x:\Kw{read}\,C | x:\Kw{mut}\,C\,\in\Gamma\}\vdash\e_0:\T$ and $\emptyset;\Gamma\vdash\e_1:\T$,
i.e. $e_0$ can be typed when seeing all externally defined mutable references as \Q@read@.

\subsection{Statement of Validation}
We first need to define what it means for an object to be valid: an object is \emph{valid} iff calling its \Q@.validate()@ method would deterministicly produce \Q@true@ in a finite number of steps, i.e. it does not evaluate to \Q@false@, fail to terminate, or produce an error.
...
${}_{}$\quad\quad$\sigma | l.validate()\Rightarrow^+ \sigma,\sigma’ | \text{\Q@true@}$

\noindent Now in order for validation to be meaningful it need’s to be possible for \Q@validate@ to potentially observe an invalid object, so we say such an observation is \emph{trusted}:
...
\noindent Finally, we can now define our main property, what it means for a language to be soundly enforce validation: every object involved in any untrusted redex is valid.

\begin{Theorem}[Sound Validation]
if $\emptyset;c:\text{\Q@mut Cap@}\vdash \e: \T$ and
$\emptyset;c:\Kw{Cap}(\_)|\e\rightarrow^+ \sigma|\ctx_v[r_l]$, then
