...
However this flexibility hampers code reasoning since object behaviour is usually nearly completely unrestricted. This is further complicated with the support OO languages typically have for exceptions, \IO{memory mutation}, and I/O.
\IO{Class invariants} are a well known technique \IO{to help write correct code}, however
...
We investigate a protocol where class-invariants are expected to hold for all objects involved in execution, but they are soundly checked only in a few points. Our approach is sound since we ensure that invariants are pure and checked immediately after an invariant violation could have occurred.

This allows for stronger and simpler reasoning, as well as less runtime verification overhead, to the more common approach of `visible state' semantics where only the invariant of \Q@this@ is required to hold, and only at the start and end of instance methods.
...
We combine previous works on type modifiers and object capabilities to ensure purity, determinism, aliasing, and mutability control. Type modifiers and object capabilities are useful in their own right, but we use these facilities here to ensure our \IO{sparsely} injected run-time checks are sufficient. 
\LINE
...
Through examples, we show how hard it is to reason about code behaviour in the context of dynamic dispatch and I/O. We show how type modifiers and object capabilities lay down the foundation needed to reason about OO code in this context, and that without such reasoning, enforcing class-invariants for all objects involved in execution becomes nearly impossible.
We show that type-modifiers and object capabilities allow for such reasoning to be both simple, modular, and sound in a context where I/O, exceptions, and dynamic-dispatch are present.

\loseSpace
\noindent\textit{Grounding:} %What argument, feasibility proof, artifacts, or results and evaluation support this work?
We demonstrate, by means of a case study, empirical evidence of the conciseness of our approach in comparison to Spec\#.
We also show the performance of our protocol in comparison to the conventional runtime-verification approaches taken by D and Eiffel.

We formally model a class of languages that support our protocol. This model is formally verified by a proof, parametric on the presence of a type system which guarantees certain properties of type modifiers. Type systems with such properties have already been explored, formalized, and proved in prior work.
...
In addition our approach works independently of the behaviour of potentially buggy or malicious code, provided it of course type checks. Since our approach also works on existing emerging languages, and adds only marginal program complexity, we believe it is also practical to be used. 
