...
\newcommand\REVComm[1]{\REV{\footnote{\REV{REV{:} #1}}}}

\newcommand{\IOComm}[1]{{\scriptsize \textcolor{blue}{[Isaac{:} #1]}}}
\newcommand{\IO}[1]{\textcolor{blue}{#1}}
\begin{document}
\paperdetails{
...
Validation is a stronger and simpler variant of `class invariants': validation always holds, while
class invariants allow objects to temporarily be in a ‘broken’ state.
(It is however easy to build class invariants over
validation).
...
For example, static verification often restricts dynamic class loading.
...
Type-modifiers and object capabilities are useful in their own right, but we use these facilities here to ensure validation by soundly injecting runtime-validation code.
...
We first provide an informal argument justifying that our rules enforce validation, in particular we show that without type modifiers and object capabilities  validation becomes nearly impossible. We also 
formally \IO{model a class} of languages that soundly support validation; this model is formally verified by a proof, parametric on the presence of a type system which guarantees certain properties of type-modifiers.
Type systems with such properties have already been explored, formalized, and proved in prior work.
...
Our work represents a \REV{philosophical shift} similar to the jump from unchecked casts in C to checked casts in Java: it is the programmer's responsibility to create valid objects and to preserve validation
