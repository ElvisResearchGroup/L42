...
We wish to guarantee that class-invariants hold for all observable instances of such a class.
We wish to ensure simpler and stronger reasoning over the the usual convention that class invariants 
need only hold at the boundaries of instance methods.

Most prior work on enforcing class invariants assumes unverified/unverifiable restrictions on library code, 
...
reason about OO code in this context. Without such reasoning enforcing class-invariants becomes nearly objects.
We show, that in comparison to other approaches, type-modifiers and object capabilities allow for 
such reasoning to be simple.

\loseSpace
\noindent\textit{Grounding:} %What argument, feasibility proof, artifacts, or results and evaluation support this work?
We formally model a class of languages that soundly enforces that the class-invariants of all observable objects hold.
This model is formally verified by a proof, parametric on the presence of a type system
which guarantees certain properties of type modifiers. 
