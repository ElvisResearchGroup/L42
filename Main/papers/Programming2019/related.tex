...
it seems that most RV\REVComm{used before it is defined} works have a philosophy that is radically different from ours;
...
However, we think this would be a \REV{terrible} idea causing unexpected strange behaviour, where some but not all
...
We believe our sound approach can monitor programs
...
To solve this issue, static verification systems are starting to\REVComm{[is this correct?] verification of reference monitors, gradual typing, and contracts have been explored for longer} consider a verified core
...
Consider for example the work of Matthew~\cite{parkinson2007class}\REVComm{[Change to] Parkinson}:
