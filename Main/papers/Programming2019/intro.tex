...
% We have performed a simple case study, showing that runtime verification of visible state semantics can cause severe performance overhead. We compared this to the pack/unpack approach used by Spec\#, though the runtime-overhead 

% However, this approach requires minimal annotation b
...
Here we propose a technique with the efficient of pack/unpack, but with a much lower annotation burden.
\LINE

Reasoning about imperative object oriented (OO) programs is a non trivial task,
made particularly difficult by mutation, aliasing, dynamic dispatch, I/O, and exceptions. \IO{There are many ways to perform such reasoning, here we use the type-system to restrict, but not prevent, such behaviour of OO in order to be able to soundly enforce class-invariants with runtime-verification (RV).}
