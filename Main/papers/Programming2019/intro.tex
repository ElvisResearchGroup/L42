...
\noindent Though this is very similar to enforcing class invariants, there are a few key differences:
\begin{itemize}
\item Class invariants can be \emph{temporarily} broken during instance methods, whereas validation doesn’t allow this.
\item \REVRComm{It is the responsibility of the class \emph{author} to ensure the invariant is preserved across all public methods, whereas with validation it is the responsibility of the class \emph{user} to not (indirectly) mutate the object in any way that will break it}{2}{What does this mean?}.
\end{itemize}
...
\REVRComm{Indeed, class invariants can be represented in our system by defining validity as 
...
many \REVRComm{conventional programming patterns are disallowed}{2}{[What] patterns are disallowed?},
...
\noindent Our approach follows the principles of \REVRComm{\emph{Defensive/Offensive programming}~\cite{WikiDefensive}}{2}{Is [this citation] a (or the most) reliable source of defensive programming principles?}:
\begin{itemize}
\item The failure (an unchecked exception)
is raised \REVRComm{as close as possible to the defect}{3}{[meaning] is not clear} (the operation creating an invalid object), i.e. we ``fail-fast''.
...
however, in Java casts are soundly checked: if in a Java program the control flow \REVRComm{goes over a cast}{2}{unclear about whether it describes a cast that succeeds or fails/},
