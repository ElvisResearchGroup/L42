...
\IO{\textit{Example:} we now show a simple example, where we have a \Q@Cage@ class containing a path (a list of \Q@Point@s) and \Q@Hamster@ with a location (a \Q@Point); the \Q@Cage@ moves the \Q@Hamster@ across it's path. \Q@Cage@ has an invariant: the \Q@Hamster@'s location is within the \Q@Cage@'s path.
}
...
\end{lstlisting}
Many verification approaches take advantage of the separation between value types and objects.
\IO{However, our approach works in a pure OO setting (such as L42) without value types.} Hence we write \IO{all type} names in upper case, to underline this. Note: to save space, here and in the rest of the paper we omit the bodies of constructors that simply \IO{initialise fields with the value of constructors parameters}.
...
The \Q@moveTo@ method is the only place in the above code where the invariant may be broken. However, there is only a single occurrence of \Q@this@ used to read the \Q@h@ field. \IO{We leverage} over TMs to ensure that no alias to \Q@this@ could be reachable from either \Q@h@ or the immutable \Q@Point@ parameter. Thus, the potentially broken \Q@this@ object is not visible while the \IO{\Q@Hamster@'s} position is updated. 

The invariant is checked at the end of the \Q@moveTo@ method, just before \Q@this@ would become visible again.
This technique loosely corresponds to an implicit pack \IO{\&} unpack: we use \Q@this@ only to read the field value, then we work on it's value while the invariant of \Q@this@ is not known to hold\IO{; finally} we check the invariant before allowing the object to  be used again.
...
% This is not the case in Spec\#: all code involved in  verification needs to be designed with verification in mind.
...
\IO{\textit{Spec\# example:}} Here we show the previous example in Spec\#, the system most similar to ours\footnote{\TODO{An extended discussion on why we think this is the best possible solution in Spec\# can be found in Appendix XX.}}:
...
\end{lstlisting}
\IO{In both versions, we designed \Q@Point@ and \Q@Hamster@ in a general way, and not soley to be used by classes with an invariant, in particular \Q@Point@ is not an immutable class. However, doing this in Spec\# proved difficult, in particular that were were unable to override \Q@Object.Equals@, or even define a usable \Q@equals@ method that takes a \Q@Point@, as such we could not call either \Q@List<Point>.Contains@ or \Q@List<Point>.IndexOf@.}
 
\noindent Even with all the above annotations, we still \IO{needed special care in creating \Q@Cage@s}:
\begin{lstlisting}[
%basicstyle=\footnotesize,
language={[Sharp]C}, morekeywords={invariant,ensures,requires,expose,exists}]
...
\IO{\textit{Contents of this paper:}}
\IO{We have fully implemented our protocol in L42\footnote{\IO{An experimental version of L42 supporting the protocol described in this paper, together with the full code of our case studies is available at \TODO{\url{...}}.}}\footnote{\IO{We also believe it would be easy to implement our protocol in Pony and Gordon et. al.'s language.}}, we used this implementation to implement and test an interactive GUI involving a class with an invariant. On a test case with $4$ objects with an invariant, 
our protocol proved to require over half a million times less checks than visible state semantics. We also compared with Spec\#, whose invariant protocol provides the same performance as ours, however the annotation burden was almost 4 times higher than our approach.
...
Sections \ref{s:related} and \ref{s:conclusion} provides a summery of related, and our work (respectively).
