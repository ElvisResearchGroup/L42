...
\MS{\textit{Example:} we now show a simple example, where we have a \Q@Cage@ class with a list of \Q@Point@s \Q@path@ and \Q@Hamster@ with a \Q@Point@ \Q@pos@; the \Q@Cage@ moves the \Q@Hamster@ across it's path. \Q@Cage@ has an invariant: the \Q@Hamster@'s location is within the \Q@Cage@'s path.}
...
\end{lstlisting}
Many verification approaches take advantage of the separation between \MS{primitive value types} and objects.
\MS{However, our approach works in a pure OO setting without such distinction.} Hence we write \IO{all type} names in upper case, to underline this. Note: to save space, here and in the rest of the paper we omit the bodies of constructors that simply \IO{initialise fields with the value of constructors parameters}.
...
The \Q@moveTo@ method is the only place in the above code where the invariant may be broken. However, there is only a single occurrence of \Q@this@ \MS{and it is} used to read the \Q@h@ field. \IO{We leverage} over TMs to ensure that no alias to \Q@this@ could be reachable from either \Q@h@ or the immutable \Q@Point@ parameter. Thus, the potentially broken \Q@this@ object is not visible while the \IO{\Q@Hamster@'s} position is updated. 
The invariant is checked at the end of the \Q@moveTo@ method, just before \Q@this@ would become visible again.
This technique loosely corresponds to an implicit pack \MS{and} unpack: we use \Q@this@ only to read the field value, then we work on it's value while the invariant of \Q@this@ is not known to hold\IO{; finally} we check the invariant before allowing the object to  be used again.

\MS{Note: since only \Q@Cage@ has an invariant,
...
\IO{\textit{Spec\# example:}} Here we show the previous example in Spec\#, the system most similar to ours \TODO{(see Appendix X for a more detailed discussion about this solution)}:
...
\end{lstlisting}
\IO{In both versions, we designed \Q@Point@ and \Q@Hamster@ in a general way, and not soley to be used by classes with an invariant, in particular \Q@Point@ is not an immutable class. However, doing this in Spec\# proved difficult, in particular \MS{we} were unable to override \Q@Object.Equals@, or even define a usable \Q@equals@ method that takes a \Q@Point@, as such we could not call either \Q@List<Point>.Contains@ or \Q@List<Point>.IndexOf@.}
