...
%##what is validation
A class may provide a \Q@Bool validate()@ method, encoding the expectations all instances should satisfy; those that do not, have an implicit default implementation of \Q@{return true;}@, i.e. every instance is valid.
...
\end{itemize}
\noindent Our system is more general and allows global reasoning through the program. In particular, class-invariants can be represented in our system by defining validity as the class-invariant holding, and a mutator-method not currently running (this is easily implemented with a simple boolean flag).
...
This approach contrasts with an alternative approach, that of static verification, in which case a program that could produce invalid objects would fail to compile. Our approach is more flexible as it allows for a program that may produce invalid objects to still be useful, with the programmer only needing to handle them when they occur.
