...
Recently, a new design for \emph{type modifiers} (TMs) has emerged that radically improves their usability.
Three different research languages are being independently developed relying on this new design: \IO{Gordon}~\cite{?}, Pony~\cite{?} and L42~\cite{?}. Those modifiers offer us a simpler way to perform high-level reasoning on OO programs, without restricting dynamic class loading or employing a verification language.
...
\MS{\emph{Object capabilities} (OCs) are a complementary technique }
...
\loseSpace
We propose a way of soundly verifying class invariants by using a minimalistic code instrumentation strategy, which can be applied to languages like \IO{Gordon}, L42 or Pony without adding any additional syntax, nor changing their type systems.
...
Our approach has 2 main benefits over the traditional one: it greatly simplifies reasoning since programmers can assume that objects accessible to them are valid\footnote{
...
\IO{In particular, TMs only  require reasoning about encapsulation locally in classes with an invariant over such encapsulated state.} In our case study, Spec\# required 3 times more annotations than our approach.
%TODO improve?
