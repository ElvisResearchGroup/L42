...
(sometime called isolated/iso~\cite{GordonEtAl12,clebsch2015deny,clebsch2017orca}), where:
\begin{itemize}
...
For the purposes of validation we only need to restrict mutation, so we may relax previous work where capsules are `balloons'~\cite{Almeida97,ServettoEtAl13a} in the object graph, and allow the ROG of a \Q@capsule@ to be referenced as \Q@read@ from outside.
...
\item A capsule field access has the same type modifier of the receiver.
...
(this requires some restrictions on \Q@read@ fields that we do not discuss in details for lack of space).
\end{itemize}
...
Using the capsule field, our \Q@ShippingList@ class, after automatic insertion of validation checks, could look like:
\saveSpace
\begin{lstlisting}
class ShippingList{
  imm Int capacity; // how much weight the ship can withstand
...
\noindent The previous restrictions ensure that capsules fields are essentially an exclusive mutable reference.
...
the method's execution, we could not observe it until after method completion.
At first glance, it may seams that capsule mutators allows only very limited kinds of mutations.
...
imm Void method imm Void doStuff(read C c) {
  ...
  }//`i' is captured. We can capture only `imm' and `capsule'
\end{lstlisting}
...
\Q@.doStuff()@ is \emph{not a capsule mutator method}, so it is not constrained by the restrictions on capsule mutators. Code like the above would also allow one to mutate multiple capsule fields in one method.
Our pattern cooperate with the language restrictions to ensure each mutation is completed as a separate operation.%
...
However the limitations of capsule mutator methods means that any \Q@mut@ methods in \Q@ListBox@ taking \Q@read@ or \Q@mut@ parameters or returning \Q@mut@, cannot be trivially forwarded.
...
To validate instead a list of \Q@mut@ objects we would need to use our box pattern not just around the list,
