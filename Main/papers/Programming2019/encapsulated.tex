...
class ShippingList {
  ...
  read method imm Bool validate() {	%\Comment{to heavy and the ship may sink}%
...
$\Comment{l is ok now, but we can break it}$
evilAlias.addItem(new HeavyItem()); $\Comment{l is now invalid as the item's are too heavy}$
...
$\Comment{l is ok now}$
mut Items evilAlias = l.items $\Comment{here l loses control}$
$\Comment{l is still ok}$
evilAlias.addItem(new HeavyItem()); $\Comment{we invalidated l now}$
\end{lstlisting}
\saveSpace
...
class ShippingList{
  ...
  ShippingList(capsule Items items) {
    this.items = items; $\Comment{mutation of capsule field}$
    if (!this.validate) {throw Error(...);} $\Comment{check at end of constructor}$
  }
  read method imm Bool validate() {	$\Comment{too heavy and the ship may sink}$
    return this.items.weight() <= this.capacity;
  }
  mut method imm Void addItem(imm Item item) {
    this.items.add(item); $\Comment{mutation of a capsule field}$
    $\Comment{check at end of capsule mutator}$
...
mut method imm Void multiThis(imm C c) {
  ...
  f.hi(); %\Comment{can it observe a broken this?}%
}
\end{lstlisting}
\saveSpace
...
mut method imm Void break(read Foo f) {
  this.items.add(new HeavyItem())
  f.hi() %\Comment{Can `this' be observed here?}%
}
...
mut ShippingList l = new ShippingList();
read Foo f = new Foo(l);
l.break(f); %\Comment{We pass another reference to `l' through f}%
\end{lstlisting}
\saveSpace
...
At first glance it may seem like our restrictions on capsule mutators
\saveSpace
\begin{lstlisting}
interface ItemTransformer { 
  imm method imm Void transform(mut Items items);
}
class ShippingList {
  ...
...
\end{lstlisting}
One could then use this general \Q@transformItems@ method to perform arbitrary mutations on a \Q@ShippingLists@'s items, as shown below:
\begin{lstlisting}[escapechar=\%]
imm Void method imm Void doStuff() {
 %\Comment{Lambda Expression that creates a new ItemTransformer}%
 this.transformItems(i -> {
    i.add(new HeavyItem());
    %\Comment{}%this.items.clear();%\Comment{Can't capture `this' and is it can't be coerced to imm}%
 })
}
\end{lstlisting}
Note that since \Q@doStuff@ does not access a capsule field (it merely calls a method that does) it is not itself a capsule mutator, so none of those restrictions apply to it. Code like the above would also allow one to mutate multiple capsule fields in one method, however each such mutation must be independent, i.e. they can't see the other capsule fields.