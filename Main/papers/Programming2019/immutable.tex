...
Here we assume the \REVRComm{full/deep meaning}{2}{Are these terms used from these prior works? Cite them}:
\begin{itemize}
  \item \REVRComm{the objects in the ROG of an immutable object are immutable;
  this corresponds to UML DataTypes}{3}{what is the reason behind comparing ROG immutability with UML DataTypes},
...
variable declared outside of such expression does not modify objects visible outside. This consideration is particularly useful to understand code in the presence of exceptions. Other authors have identified the concept of \REVRComm{Strong Exception Safety~\cite{Abrahams2000} as a general issue when reasoning about objects state after catching an exception:
when a \Q@try-catch@ catches an exception, the visible objects must be the same as before the \Q@try@ block started its execution.}{2}{compare to the notion of transaction}
...
\loseSpace
\noindent\REVRComm{\textit{Object Capabilities:}}{2}{Citations here?}
...
In a language enforcing object capabilities the \Q@main@ method could take a \Q@System@ object as a parameter, and using that object is the only way to perform I/O, as in \REVRComm{\Q@mySystem.println("hello")@}{3}{Incorrect variable name}.
...
\section{\REVRComm{The \Q@.validate()@ Method remind of the goals for validate (i.e. that it be pure)}{2}{What about possible non-termination of validate()}}
\label{s:validate}
...
\noindent \REVRComm{Also note \Q@.validate()@ is not declared as throwing any exceptions, thus it can only leak unchecked exceptions.}{3}{Can ``leaking'' of an unchecked exceptions have negative consequences for validity? Leaking is usually a negative word used in a negative context.}


Clearly the \Q@.validate()@ method must be able to take an invalid \Q@this@, since the purpose of such method is to distinguish valid and invalid objects.
...
\loseSpace
\noindent\REVRComm{\textit{Allowing internal mutations/back-doors:}}{3}{[This] paragraph and its illustration is not very clear, especially on how it can cause nondeterminism by reading/storing information about previous calls.}
% TODO: Come up with better title
\REVRComm{Suppose we relax our mutation rules, by allowing interior mutability
