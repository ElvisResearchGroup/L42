...
\item Immutable (\Q@imm@): the referenced object can never mutate (not even through an alias), and
\end{itemize}
...
class Point {
  ...
  %\Comment{Method whose ‘this’ paramater is mut}%
  mut method imm Void X(imm Int x) { 
    this.x = x; %\Comment{Ok, ‘this’ is mut so we can update a field}%
  } 

  %\Comment{Method whose ‘this’ paramater is read}%
  read method imm Int X() { 
    return this.x; %\Comment{Ok, ‘imm’ is more restrictive than ‘read’}%
  }
  ...  
}
class Circle {
  public mut Point center;
  read method read Point center() {
    return this.center; %\Comment{‘this’ is read, so ‘center’ is also read}%
  }
  ...
}
...
...
%\Comment{}%ip.X(3) %\Comment{type error, .X(imm Int) takes a mut, but ip is imm}%
read Point rp = mp %\Comment{ok read is a common supertype of imm/mut}%
%\Comment{}%rp.X(3) %\Comment{type error, rp is not mut}%
mp.X(5) %\Comment{ok}%
asssert rp.x()==5 %\Comment{suceeds, we modified rp through the alias ‘mp’}%
ip = new Point(3,5) %\Comment{ok, ip is not final}%

mut Circle mc = new Circle(mCenter, %\MComment{radius}%7)
imm Circle ic = new Circle(new Point(0,0),7) %\Comment{ok imm}%
imm Point ip = ic.center() %\Comment{ok, imm objects are deeply immutable}%

read Circle rc = mc
read Point rp = rc.center() %\Comment{ok, everything can be promoted to read}%
%\Comment{}%mut Point mp = rc.center() %\Comment{type error, rc is only ‘read’}%
\end{lstlisting}
...
This is not a special feature of the \Q@new@ construct: since \Q@new@ returns a \Q@mut@ and any expression typed as a \Q@mut@ that only uses immutable references can safely be promoted to \Q@imm@ (since the returned value could not possibly be aliased). Such flexibility is also visible where \Q@rc.center()@ returns a \Q@read@ but \Q@ic.center()@ returns an \Q@imm@ (since \Q@ic@ is \Q@imm@, and \Q@imm@ is a deep modifier).
...
Moreover, the \Q@System@ type would have no visible constructor, and all its I/O methods require a mutable (\Q@mut@) receiver.
Many other operations, like random numbers and file management could also work this way.
...
As a relaxation, we can accept to call instance methods that in turn, only use \Q@this@ to access fields, or call other such instance methods. With this relaxation, the semantics of \Q@.validate()@ need to be understood with the body of those methods inlined/macro-expanded; thus reinterpreting the semantic of the methods (namely that \Q@this@ may be invalid).
...
In this section we put consider how to validate a simple but useful category of classes. Consider the following code, where a \Q@Person@ has a single immutable (non final) field \Q@name@:
\begin{lstlisting}
class Person {
...
\end{lstlisting}
\Q@Person@, only has immutable fields and the constructor only uses \Q@this@ to access/update fields; qe say such a class is \emph{simple}.
...
The difference with respect to immutable types (like UML DataTypes) is that fields are not required to be final, thus the object can change state during its lifetime. This means that the ROGs of all the \emph{fields} are immutable, but \emph{not} the object itself.
Of course immutable types are just a special case of simple classes.
...
If we were to relax (as in Rust), or even eliminate (as in Java), the rules of TM or OC: validation of this simple \Q@Person@ class, would become harder, or even impossible.
%TODO: Move this code somewhere else
\subsubsection*{Use of object-capabilities:}
...
mut Person schrodinger = new Person(name);

schrodinger.validate(); $\Comment{would this throw an exception?}$
schrodinger.validate(); $\Comment{what about this?}$
\end{lstlisting}
Despite the code for \Q@Persion.validate()@ intuitively looking correct and deterministic, the above calls to it are not. Obviously this breaks any reasoning and makes our validation unsound. Even if we disallow subtyping the same problem could still occur if we had a strange implementing of \Q@String@, or \Q@Persion.validate()@ itself.

\subsubsection*{Internal mutations/back-doors:}
...
In this example we can use the \Q@AtomicBool@ back-door to remotely break the invariant of \Q@person@ without any interaction with the \Q@person@ object itself.
It would require some powerful static or dynamic analysis to keep track of every case the ROG of \Q@Person@ could be indirectly mutated, and insert validity checks appropriately, however ensuring deep mutability trivialises this for simple classes.


Allowing such back-doors could also be used to break the determinism of the \Q@.validate()@ method, by allowing it to potentially read information previous calls to it have stored.
