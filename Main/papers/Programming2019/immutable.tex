...
Here we assume the \REV{full/deep meaning}:
\begin{itemize}
...
variable declared outside of such expression does not modify objects visible outside. This consideration is particularly useful to understand code in the presence of exceptions. Other authors have identified the concept of Strong Exception Safety~\cite{Abrahams2000}\REVComm{compare to the notion of transaction} as a general issue when reasoning about objects state after catching an exception:
...
\section{The \Q@.validate()@ Method \REVComm{remind of the goals for validate (i.e. that it be pure)}\REVComm{What about possible non-termination of validate()}}
\label{s:validate}
...
as in Rust and Javari\REVComm{Explain why [they] allow unsafe features}: thus allowing mutation of the ROG of an immutable object through back-doors:
\begin{lstlisting}
class AtomicBool {
