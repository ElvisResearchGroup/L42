...
Moreover, its constructors may only use \Q@this@ to initialize fields.\footnote{This prevents objects from escaping their constructor partially initialised. A factory method could be used when more flexible initialisation is required.}
%Thus, in their bodies, we require that \Q@this@ is only used too read/initialise fields (respectively).
...
\IO{\section{Invariants over immutable state}}
\label{s:immState}
...
\end{lstlisting}
\Q@Person@ only has immutable fields and its constructor only uses \Q@this@ to access/update fields.
