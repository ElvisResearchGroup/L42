...
\begin{itemize}
  \item \REVComm{the objects in the ROG of an immutable object are immutable;
...
variable declared outside of such expression does not modify objects visible outside. This consideration is particularly useful to understand code in the presence of exceptions. Other authors have identified the concept of \REVComm{Strong Exception Safety~\cite{Abrahams2000} as a general issue when reasoning about objects state after catching an exception:
...
\section{\REVComm{The \validate Method remind of the goals for validate (i.e. that it be pure)}{2}{What about possible non-termination of validate()}}
\label{s:validate}
