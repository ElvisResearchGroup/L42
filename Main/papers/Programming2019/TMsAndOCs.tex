...
made particularly difficult by mutation, aliasing, dynamic dispatch, I/O, and exceptions. \IO{There are many ways to perform such reasoning, here we use the type-system to restrict, but not prevent, such behaviour in order to be able to soundly enforce class-invariants with runtime-verification (RV).}
% \IO{[dynamic class loading]},

\IO{\emph{Type modifiers} (TMs), as used in this paper, are a type system feature that allows reasoning about aliasing and mutation; recently, a new design for them} has emerged that radically improves their usability.
Three different research languages are being independently developed relying on this new design: The language of Gordon et. al.~\cite{?}, Pony~\cite{?} and L42~\cite{?}.
...
\item A reference of an \Q@imm@ type however refers to an object that will never be mutated, and neither will its \IO{reachable-object-graph (ROG)}. However, a field of type \Q@imm@ may be updated to another \Q@imm@ reference.\footnote{In C, this is similar to the difference between \Q@const *A@ and \Q@*const A@, where a \Q@final read@ variable would be like \Q@const *const A@.}
\end{itemize}
...
%cite Alex work on deep immutability
\begin{itemize}
...
\REVComm{\IOComm{I will mention about more foundational works that eventually enabled the work of Gordon}}{2}{Worth mentioning any relationship between capsules and promotion/recovery with the following? [...list of papers...]}
%such similarity is only superficial: former work infers uniqueness/isolation/immutability only when starting from references that have been tracked with restrictive annotations along their whole lifetime.
...
OCs, which L42, Pony, and Gordon et. al.'s work have, are a widely used~\cite{???} programming style that allows associating resources with objects. \MS{When} this style
is respected, code \MS{that} does not possess an alias to the object cannot use the resource.
...
\lstset{language=Java}
 ...
 \lstset{language=FortyTwo} 
  \MS{is} a \emph{capability object} that provides access to the standard-input resource, however as it is globally accessible it completely prevents reasoning about determinism. 
