...
OCs, which L42, Pony, and Gordon et. al.'s work use, are a widely used\TODO{~\cite{???}} programming style that allows associating resources with objects. When this style
...
For the purposes of invariant checking, we only care about the effects that methods could have on the running program and heap. As such, \emph{output} native methods (such as ones that print to the terminal) can be white-listed as `deterministic', provided they does not effect program execution, such as by non deterministically throwing I/O exceptions.
