...
With slightly different names and semantics, the three most common modifiers for object references are:
...
%  \item no \emph{down}-casting is allowed between different type modifiers.
  \item no \emph{down}-casting from \Q@read@ to \Q@mut@ is allowed.
...
mindlessly using \Q@mut@ types and without worrying about rampant
aliasing. Then, at a later stage, another programmer may still 
...
The \Q@capsule@ modifier (sometimes called isolated/iso) is possibly the one where more details differs in literature. Here we refer to its interpretation after the work~\cite{GordonEtAl12}, that introduced the concept of recovery/promotion.
This concept is at the basis of M\#, L42 and Pony type system~\cite{GordonEtAl12,ServettoEtAl13a,ServettoZucca15,ServettoEtAl13a,clebsch2015deny,clebsch2017orca}.
Recovery allows to takes a mutable reference and promote it to \Q@capsule@.
While at first glance this may seams similar to what former work was doing \REVComm{[....]}
{2}{Worth mentioning any relationship between capsules and promotion/recovery with the following? [...list of papers...]}
those similarity are only superficial: former works allows to infer uniqueness/isolation only when starting from references that has been tracked with restrictive annotations along their whole lifetime.
...
OCs, which M\#, L42 and Pony have, are a widely used~\cite{???} programming style that allows associating resources with objects. \MS{When} this style
