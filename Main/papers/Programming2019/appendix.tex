\section{Description of our case studies}
\subsection{GUI}

We have 21 widgets.
This includes a top level (green) component and
4 movable containers.
Each container has 4 gray buttons one for each cardinal direction. When the button is pressed, the container is moved 
a little bit in the corresponding direction. 
The containers can also contain other widgets; in our
set up there is a big red container containing 2 smaller blue containers. One of those contains a tiny black container.
...
We created an automated test pressing each of the 16 buttons onece.
In L42, using the approach of this paper, this calls the invariant 77 times and ...
Instead, using the conventional approach, it calls the invariant ... times (and it took 4 days to run)

It can be surprising to see such result.
We runned our example with less widgets, and our results suggest an exponential grouth in the cost of the conventional approach. For example by removing 2 containers (and their 8 buttons) we get....

We now explain how this exponential explosion happens:

For the outer-most box to check its invariant, it needs to call methods left,top,height and with to all its contained widgets.
If one of those widget has invariant, when such public method is called,
its invariant is checked (twice). This requires to call left,top,height and witdh to all its contained widgets, some of those may also have invariants.
// Isaac will do a better explanation of why the CM is exponesional, but our's is only quadratic

In literature there is attention to prevent method called from an invariant to call public methods on 'this'; this would cause the system to just go in loop.
However there is no limitation on calling methods on other objects, and if those objects have invariant, this would cause the aforemention explosion. 

Our example also shows that the restrictions of TM's and OC's are flexible enough to encode interactive GUI programs, where widgets may circularlly reference other widgets.
...
* they check the admissibility/valididty of a the invariant code
...
with purity and ownership annotations. The ownership annotations included paramater, method and field attributes as well as requires, ensures and modifies clauses, as well as explicit onwership assignment statements.
Since Spec\# annotation can be involded, as for example ...
we are goint to count the number of tokens (excluding \Q@.@ and parenthesis).
This gave us a total of NUM tokens, that is more then 3 times the amount needed in L42.
Moreover, in L42 we only use 2 diffent kinds of annotations (mut/capsule), while in Spec\# we use ... kinds of annotations. This corroborate our statement that our system is easier to use for programmers that are not experts in sw verification.
Counting the number of charactes we get .... against ... in L42.  

We encoded our Widget example also on Microsoft Code Contract; their system also runned the invariant checking 77 times. Their system is easy to use, but it is unsound since it is built over an unsound/uncomplete static verifier [?].
...
\begin{enumerate}
	...
	\begin{itemize}
	 ...
	 $\e_2=l$\validate, thus the first execution step
