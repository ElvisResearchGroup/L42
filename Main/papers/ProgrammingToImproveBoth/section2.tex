...
Later, in Appendix \ref{sec:formal} we will provide the type
system. 
...
\MS{in this way it can be typed in isolation before flattening.}
However, this results in
many redundant abstract method declarations.

\paragraph{Semantics of Use:}
\MS{Trait semantic is defined with flattening:
 \Q@A@ and \Q@B@ are equivalent to the inlined code of all used traits{.}}%
\footnote{\MS{
The flattening semantic is the simplest trait
semantic to formalize and understand.
However, if implemented naively, it may causes a lot of
bytecode duplication.
Delegation semantic%
~\cite{lagorio2009featherweight},
is a proposed alternative semantic models for traits
that is observationally equivalent to flattening but do not require bytecode duplication. The formalism presented here will rely on simple flattening; we expect the techniques of \cite{lagorio2009featherweight} would be useful to produce an efficient implementation in term of bytecode space.
}}%
...
overridden. If too many methods do not provide an appropriate behaviour, implementing another source or
re implementing the behaviour from scratch may also be considered.
This should not impact the interface exposed to the user, otherwise the programmer may be unable to change its implementation decisions at a later time.
}}
%
