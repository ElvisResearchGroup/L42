...
\saveSpace
In Appendix A we formalise \name; \MS{in addition to conventional soundness,} we 
...
\MS{This property ensures that flattening strictly reduces the number of typing errors.
In turn, this
ensure that reusing a trait can not induce new type errors.
Those properties was already proved in~\cite{servetto2014meta}; here
the proof is smother thanks to our simpler formalization.
}
...
Cecil syntactic sugar counters this issue; advantages and pitfalls are discussed in Appendix B, \MS{especially B.2}.
...
\MS{TR, DJ and PT are research projects, aiming to 
be a platform to experiment concepts, not to expose a compact syntax to the programmer;
...
PT is built on top of Java, thus, it \MS{must support many different syntactic forms}.
\MS{\name} relies on DJ's approach but,
thanks to our novel representation of state, \name offers a much simpler and uniform syntax \MS{than
DJ, TR and PT}.
\item 
...
Looking at this examples makes it clear
 why we need reusable code to be agnostic of its name as in  TR, PT \MS{and \name}:
\MS{either reusable code does  not correspond to a type name (as in TR, PT and \name)
...
and makes it easy to extend languages with more operators. \MS{\name,} DJ and TR embrace this idea, while \uwave{PT} takes the traditional Java/C++ approach of using an enhanced class/package declaration syntax.
...
While \MS{all of \name, TR, DJ and PT} allow separating inheritance and subtyping only \MS{\name and} \textbf{TR} properly enforces 
...
\saveSpace
The original trait model has no self \MS{instantiation} 
...
Self \MS{instantiation} is also not possible 
...
This approach is designed in DJ to allow self \MS{instantiation}.
