...
Suppose Java code-reuse (the {\bf extends} keyword) was not introducing subtyping\footnote{
...
We synthesize and simplify the best ideas from those 3 very different designs, and couple them with an elegant \textbf{novel approach} to state and \MS{self construction} in traits that avoids the complexities and redundancies introduced by fields and their initialisation.
%-------
...
\MS{We will show how this allows fine control of subtyping while handling examples like \Q@Set/Bag@.}

\MS{In \name, as in many module composition languages~\cite{ancona_zucca_2002},
static methods are also dynamically dispatched and can even be abstract. In this context, an abstract static method can perform the role of a constructor.
We will show how in \name, state is induced by an implicit fixpoint operation over abstract methods.
This allow to handle examples like \Q@Point/ColorPoint@ in a natural way, and without requiring code duplication.}
