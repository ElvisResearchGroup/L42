...
Finally, when self construction is not needed, declaring the abstract factory can be made redundant, as is done in the full 42 language.

Moreover, when code reuse is involved, \name is much simpler to use then Java, C++ or C\#: a class extending another class usually needs to define a constructor calling a super constructor.
...
Just looking at how involved it is to implement an immutable \Q@Point@ class in Java with withers, it is sufficient to dissuade most programmers.

\subsection{Comprehensibility and usability}
Of course it could take a while to adapt from the Java model to the \name one. However, when it comes to model complex reuse scenarios, we believe \name is much more explicit and clear then Scala and other options. 
%We invite the curios reader to look to the complete code of the case studies (below) and compare our solution with the cryptic version of Scala, where behaviour traits are nested inside other traits.
...
\item 
% The runtime semantic do not depend on the Our object model is equivalent to an untyped language.
...
\subsubsection{Java7}
\lstset{language=Java,basicstyle=\tiny\ttfamily,numberstyle=\tiny\ttfamily}
\begin{lstlisting}
//case 0 of 16
class Point {//6 lines
  final int x;
...
\end{lstlisting}
\subsubsection{Classless Java}
\begin{lstlisting}
// 0
@Obj // 3 lines
interface Point {
  static Point of(int x, int y) { // "of" methods are generated by @Obj thus their lines are not counted
    return new Point() {
      public int x() {
        return x;
      }
...
interface PointSumMul extends PointSum, PointMul {
  ...
  default PointSumMul sum(Point that) {// we have to rewrite the method to call the new "of"
...
interface PointSumSub extends PointSum, PointSub {
  ...
  default PointSumSub sum(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() + that.x(), this.y() + that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSumDiv extends PointSum, PointDiv {
  ...
  default PointSumDiv sum(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() + that.x(), this.y() + that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSubMul extends PointMul, PointSub {
  ...
  default PointSubMul mul(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() * that.x(), this.y() * that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSubDiv extends PointSub, PointDiv {
  ...
  default PointSubDiv sub(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() - that.x(), this.y() - that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointMulDiv extends PointMul, PointDiv {
  ...
  default PointMulDiv mul(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() * that.x(), this.y() * that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSumMulDiv extends PointSumMul, PointMulDiv, PointSumDiv {
  ...
  default PointSumMulDiv mul(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() * that.x(), this.y() * that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSubMulDiv extends PointSubMul, PointMulDiv, PointSubDiv {
  ...
  default PointSubMulDiv mul(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() * that.x(), this.y() * that.y());// in order to produce an instance of PointSumMul
  }
...
interface PointSumSubMulDiv extends PointSumSubMul, PointSumMulDiv, PointSumSubDiv, PointSubMulDiv {
  ...
  default PointSumSubMulDiv mul(Point that) {// we have to rewrite the method to call the new "of"
    return of(this.x() * that.x(), this.y() * that.y());// in order to produce an instance of PointSumMul
  }
...
%Note: we do not refine method sum return type in many cases. THIS IS AN ERROR!
\lstset{language=Java, morekeywords={enum}}
\begin{lstlisting}
enum Flavor {
...
//glue code from now on
class CPoint(val x:Int, val y:Int, val color:Int) extends tPointSum with tColored {//12 lines
...
\subsubsection{Isolated Scala}
The following code is fully modularized:
