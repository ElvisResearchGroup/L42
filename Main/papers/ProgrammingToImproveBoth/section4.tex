...
\MS{by implementing methods as constructor calls and field access/update}
  is a
...
\item \MS{A class with a single abstract \Q@static@ method 
...
\footnote{
Since we want to focus on the actual code, while counting line numbers we \emph{omit} empty lines and lines containing only open/closed \MS{parenthesis/braces}.
}
...
operations and finally $11$ lines for the class with all four operations.
 ...
 \MS{sum, sub and other} operations in each of the classes. Here the limited
...
\MS{are formatted for compactness}.
...
\saveSpace\saveSpace 
\noindent 
where we assume support for overloading \MS{based on} different numbers of parameters.
...
\MS{We can design trait \Q@p@ for reuse and extensibility
...
Java.
For this example, in Java we encode \Q@Point@ with \MS{the fields but} no operations,
...
to allow self instantiation as in~\cite{Zenger-Odersky2005}.}
 allow supporting the `\Q@This@' type and `\Q@This@' type instantiation but they require a lot of set-up code. We experimented those patterns, but it soon became very clear that the resulting code of this approach would have been even larger; albeit without duplicated code.
